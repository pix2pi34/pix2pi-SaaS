...
import { fetchMonitoringContract } from '../api/monitoringApi'
import type { MonitoringContractData } from '../contracts/monitoring.contract'

type MonitoringSummaryState = {
  status: 'loading' | 'success' | 'error'
  data: MonitoringContractData | null
  errorMessage: string
  requestId: string
  source: string
  refresh: () => void
...
export function useMonitoringSummary(tenantCode?: string): MonitoringSummaryState {
  const effectiveTenantCode = normalizeTenantCode(tenantCode)
  const [refreshKey, setRefreshKey] = useState(0)
  const [status, setStatus] = useState<'loading' | 'success' | 'error'>('loading')
  const [data, setData] = useState<MonitoringContractData | null>(null)
  ...
  useEffect(() => {
    ...
    void fetchMonitoringContract(effectiveTenantCode).then((response) => {
