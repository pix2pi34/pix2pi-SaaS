...
type MaintenanceSQLStore struct {
	db QueryRowProvider
}

func NewMaintenanceSQLStore(db QueryRowProvider) *MaintenanceSQLStore {
	return &MaintenanceSQLStore{
		db: db,
	}
}

func (s *MaintenanceSQLStore) RequestMaintenanceAction(ctx context.Context, cmd RequestMaintenanceActionCommand) (RequestMaintenanceActionResult, error) {
	if s == nil || s.db == nil {
		return RequestMaintenanceActionResult{}, errors.New("maintenance sql store hazir degil")
	}
...
inserted_action AS (
  ...
  SELECT
    ...
    now(),
    CASE
      WHEN $8 THEN 'dry-run maintenance requested'
      ELSE ''
    END,
    jsonb_build_object(
      'dry_run', $8,
...
`
	...
	var result RequestMaintenanceActionResult
