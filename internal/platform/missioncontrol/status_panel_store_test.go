...
func TestSQLStoreListRuntimeStatusCards_Success(t *testing.T) {
		...
		rows: &rowsMock{
			rows: [][]any{
				...
				},
				{
					...
					9010,
					"1.0.0",
					time.Date(2026, 4, 24, 21, 1, 0, 0, time.UTC),
				},
			},
		},
	}
	...
	items, err := store.ListRuntimeStatusCards(context.Background(), StatusPanelRequest{
		TenantID:       "tenant-a",
		IncludeGlobal:  true,
		ServiceKeyLike: "id",
		StatusFilter:   "",
...
func TestSQLStoreListRuntimeStatusCards_NoDB(t *testing.T) {
	store := NewSQLStore(nil)

	_, err := store.ListRuntimeStatusCards(context.Background(), StatusPanelRequest{
		TenantID:      "tenant-a",
		IncludeGlobal: true,
		Limit:         10,
	})
...
func TestSQLStoreListRuntimeStatusCards_QueryError(t *testing.T) {
	db := &queryRowsProviderMock{
...
func TestSQLStoreListRuntimeStatusCards_ScanError(t *testing.T) {
	db := &queryRowsProviderMock{
		rows: &rowsMock{
