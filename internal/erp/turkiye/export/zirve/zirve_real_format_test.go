...
func validConfig() RuntimeConfig {
	return RuntimeConfig{
		RuntimeEnabled:          true,
		TargetSystem:            TargetSystemZirve,
		FormatVersion:           ZirveFormatV1,
		...
		AllowedFileTypes:        []ZirveFileType{FileTypeZirveJournalTXT, FileTypeZirveLedgerTXT, FileTypeZirveSummaryTXT},
		RequiredAccountPrefixes: []string{"120", "191", "320", "391", "600", "610", "102", "153", "500"},
	}
}

func validRequest() ZirveExportRequest {
	return ZirveExportRequest{
		TenantID:       "tenant-001",
		CorrelationID:  "corr-zirve-export-001",
		RequestID:      "req-zirve-export-001",
		IdempotencyKey: "idem-zirve-export-001",
		ExportID:       "zirve-export-001",
		TargetSystem:   TargetSystemZirve,
		FormatVersion:  ZirveFormatV1,
		PeriodCode:     "2026-05",
		FiscalYear:     2026,
		Postings:       []postingruntime.PostingEntry{validPosting()},
		RequestedBy:    "zirve-export-test",
...
func newRuntime(t *testing.T) *ZirveRealFormatRuntime {
	t.Helper()

	runtime, err := NewZirveRealFormatRuntime(validConfig())
	if err != nil {
...
func TestBuildZirvePackageReady(t *testing.T) {
	runtime := newRuntime(t)

	pkg, err := runtime.BuildPackage(validRequest())
	if err != nil {
		t.Fatalf("build Zirve package failed: %v", err)
	}
...
func TestZirveJournalFileContainsExpectedRows(t *testing.T) {
	runtime := newRuntime(t)
	...
	if !strings.Contains(journal, "TARIH|FIS_NO|SATIR_NO|BELGE_NO|HESAP_KODU") {
		t.Fatal("expected Zirve journal header")
	}
...
func TestZirveLedgerAndSummaryFilesGenerated(t *testing.T) {
	runtime := newRuntime(t)
	...
	for _, file := range pkg.Files {
		if file.FileType == FileTypeZirveLedgerTXT {
			ledgerFound = true
			if !strings.Contains(file.Content, "HESAP_KODU|HESAP_ADI|BORC|ALACAK|PARA_BIRIMI") {
				t.Fatal("expected Zirve ledger header")
			}
		}
		if file.FileType == FileTypeZirveSummaryTXT {
			summaryFound = true
			if !strings.Contains(file.Content, "BALANCED|true") {
				t.Fatal("expected balanced Zirve summary")
			}
		}
	}

	if !ledgerFound {
		t.Fatal("expected Zirve ledger file")
	}
	if !summaryFound {
		t.Fatal("expected Zirve summary file")
	}
}
...
func TestNormalizeTurkishCharacters(t *testing.T) {
	got := normalizeZirveField("Çalışma Şirketi İğdır Ürün", true, 100)
...
func TestRuntimeRejectsDisabledConfig(t *testing.T) {
	...
	_, err := NewZirveRealFormatRuntime(cfg)
	if err == nil {
