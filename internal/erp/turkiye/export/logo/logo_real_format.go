...
const (
	TargetSystemLogo TargetSystem = "LOGO"
)

type LogoFormatVersion string

const (
	LogoFormatV1 LogoFormatVersion = "LOGO_TDHP_V1"
)

type LogoFileType string

const (
	FileTypeLogoJournalCSV LogoFileType = "LOGO_JOURNAL_CSV"
	FileTypeLogoLedgerCSV  LogoFileType = "LOGO_LEDGER_CSV"
	FileTypeLogoSummaryTXT LogoFileType = "LOGO_SUMMARY_TXT"
)
...
type RuntimeConfig struct {
	...
	FormatVersion           LogoFormatVersion `json:"format_version"`
	...
	AllowedFileTypes        []LogoFileType    `json:"allowed_file_types"`
	RequiredAccountPrefixes []string          `json:"required_account_prefixes"`
}

type LogoExportRequest struct {
...
type LogoExportFile struct {
	FileType  LogoFileType `json:"file_type"`
...
type LogoValidationIssue struct {
	Code     string `json:"code"`
	Message  string `json:"message"`
	Severity string `json:"severity"`
}

type LogoExportPackage struct {
	TenantID       string `json:"tenant_id"`
	CorrelationID  string `json:"correlation_id"`
	RequestID      string `json:"request_id"`
	IdempotencyKey string `json:"idempotency_key"`
	...
	JournalRows []LogoJournalRow      `json:"journal_rows"`
	Files       []LogoExportFile      `json:"files"`
	Issues      []LogoValidationIssue `json:"issues"`
...
type LogoRealFormatRuntime struct {
	config RuntimeConfig
}

func NewLogoRealFormatRuntime(config RuntimeConfig) (*LogoRealFormatRuntime, error) {
	if !config.RuntimeEnabled {
		return nil, errors.New("Logo real format runtime is disabled")
	}
	if config.TargetSystem != TargetSystemLogo {
		return nil, errors.New("target_system must be LOGO")
	}
	...
	return &LogoRealFormatRuntime{config: config}, nil
}

func (r *LogoRealFormatRuntime) BuildPackage(req LogoExportRequest) (LogoExportPackage, error) {
	...
	if r.config.StrictBalanceRequired && !balanced {
		return rejectedPackage(req, "LOGO_EXPORT_NOT_BALANCED", "Logo export debit and credit totals are not equal"), errors.New("Logo export debit and credit totals are not equal")
	}

	files := []LogoExportFile{
	...
	if len(issues) > 0 {
		pkg := rejectedPackage(req, "LOGO_VALIDATION_ISSUES", "Logo export validation issues detected")
		...
		return pkg, errors.New("Logo export validation issues detected")
	}

	return LogoExportPackage{
		...
		AuditAction:         "LOGO_REAL_FORMAT_PACKAGE_READY",
		AuditDecisionReason: "posting entries converted to Logo journal, ledger and summary files",
		CreatedAt:           time.Now().UTC(),
	}, nil
}

func (r *LogoRealFormatRuntime) ValidatePackage(pkg LogoExportPackage) []LogoValidationIssue {
	issues := make([]LogoValidationIssue, 0)

	if strings.TrimSpace(pkg.TenantID) == "" {
		issues = append(issues, issue("TENANT_ID_MISSING", "tenant_id is required", "BLOCKING"))
	}
	if pkg.TargetSystem != TargetSystemLogo {
		issues = append(issues, issue("TARGET_SYSTEM_INVALID", "target_system must be LOGO", "BLOCKING"))
	}
...
func (r *LogoRealFormatRuntime) buildJournalRows(req LogoExportRequest) ([]LogoJournalRow, int64, int64, error) {
	rows := make([]LogoJournalRow, 0)
	...
	for _, posting := range postings {
		...
		for _, line := range posting.Lines {
			...
			rows = append(rows, LogoJournalRow{
				TenantID:    req.TenantID,
				ExportID:    req.ExportID,
				PostingID:   posting.PostingID,
				PostingNo:   posting.PostingNo,
				LineNo:      line.LineNo,
				Date:        posting.DocumentDate.Format("2006-01-02"),
				DocumentNo:  normalizeLogoField(posting.DocumentNo, r.config.NormalizeTurkishChars, 30),
				AccountCode: normalizeLogoField(line.AccountCode, false, 32),
				AccountName: normalizeLogoField(line.AccountName, r.config.NormalizeTurkishChars, 80),
				Debit:       formatKurus(line.DebitAmountKurus),
				Credit:      formatKurus(line.CreditAmountKurus),
				Currency:    posting.CurrencyCode,
				Description: normalizeLogoField(line.Description, r.config.NormalizeTurkishChars, r.config.MaxDescriptionLength),
				PartyTaxNo:  normalizeLogoField(line.PartyTaxNo, false, 20),
...
func (r *LogoRealFormatRuntime) validateRequest(req LogoExportRequest) error {
	...
	if req.TargetSystem != TargetSystemLogo {
		return errors.New("target_system must be LOGO")
	}
...
func (r *LogoRealFormatRuntime) validatePosting(expectedTenantID string, posting postingruntime.PostingEntry) error {
...
func (r *LogoRealFormatRuntime) validateRows(rows []LogoJournalRow) []LogoValidationIssue {
	issues := make([]LogoValidationIssue, 0)
...
func (r *LogoRealFormatRuntime) accountPrefixAllowed(accountCode string) bool {
...
func (r *LogoRealFormatRuntime) buildJournalFile(req LogoExportRequest, rows []LogoJournalRow) LogoExportFile {
	header := []string{"DATE", "FICHENO", "LINENO", "ACCOUNTCODE", "ACCOUNTNAME", "DEBIT", "CREDIT", "CURR", "DESCRIPTION", "TAXNO", "POSTINGHASH"}
	...
	return LogoExportFile{
		FileType:  FileTypeLogoJournalCSV,
		FileName:  fmt.Sprintf("%s_LOGO_JOURNAL.csv", req.ExportID),
		MimeType:  "text/csv",
		Content:   content,
		LineCount: len(lines),
		FileHash:  buildFileHash(FileTypeLogoJournalCSV, content),
	}
}

func (r *LogoRealFormatRuntime) buildLedgerFile(req LogoExportRequest, rows []LogoJournalRow) LogoExportFile {
	header := []string{"ACCOUNTCODE", "ACCOUNTNAME", "DEBIT", "CREDIT", "CURR"}
	lines := []string{strings.Join(header, r.config.Delimiter)}
	...
	content := strings.Join(lines, r.config.LineEnding) + r.config.LineEnding
	return LogoExportFile{
		FileType:  FileTypeLogoLedgerCSV,
		FileName:  fmt.Sprintf("%s_LOGO_LEDGER.csv", req.ExportID),
		MimeType:  "text/csv",
		Content:   content,
		LineCount: len(lines),
		FileHash:  buildFileHash(FileTypeLogoLedgerCSV, content),
	}
}

func (r *LogoRealFormatRuntime) buildSummaryFile(req LogoExportRequest, rows []LogoJournalRow, totalDebit int64, totalCredit int64) LogoExportFile {
	lines := []string{
	...
	content := strings.Join(lines, r.config.LineEnding) + r.config.LineEnding
	return LogoExportFile{
		FileType:  FileTypeLogoSummaryTXT,
		FileName:  fmt.Sprintf("%s_LOGO_SUMMARY.txt", req.ExportID),
		MimeType:  "text/plain",
		Content:   content,
		LineCount: len(lines),
		FileHash:  buildFileHash(FileTypeLogoSummaryTXT, content),
	}
}

func normalizeLogoField(value string, normalizeTurkish bool, maxLength int) string {
...
func issue(code string, message string, severity string) LogoValidationIssue {
	return LogoValidationIssue{Code: code, Message: message, Severity: severity}
}

func buildFileHash(fileType LogoFileType, content string) string {
	return fmt.Sprintf("logo-file:%s:%d:%d", fileType, len(content), len(strings.Split(content, "\n")))
}

func buildPackageHash(req LogoExportRequest, files []LogoExportFile, totalDebit int64, totalCredit int64) string {
	parts := []string{
	...
	return "logo-package:" + strings.Join(parts, ":")
}

func rejectedPackage(req LogoExportRequest, code string, message string) LogoExportPackage {
	return LogoExportPackage{
		TenantID:            req.TenantID,
		CorrelationID:       req.CorrelationID,
		RequestID:           req.RequestID,
		...
		Balanced:            false,
		AuditAction:         "LOGO_REAL_FORMAT_PACKAGE_REJECTED",
		AuditDecisionReason: "Logo format package rejected by validation guard",
		ErrorCode:           code,
