...
func validConfig() RuntimeConfig {
	return RuntimeConfig{
		RuntimeEnabled:          true,
		TargetSystem:            TargetSystemMikro,
		FormatVersion:           MikroFormatV1,
		...
		AllowedFileTypes:        []MikroFileType{FileTypeMikroJournalCSV, FileTypeMikroLedgerCSV, FileTypeMikroSummaryTXT},
		RequiredAccountPrefixes: []string{"120", "191", "320", "391", "600", "610", "102", "153", "500"},
	}
}

func validRequest() MikroExportRequest {
	return MikroExportRequest{
		TenantID:       "tenant-001",
		CorrelationID:  "corr-mikro-export-001",
		RequestID:      "req-mikro-export-001",
		IdempotencyKey: "idem-mikro-export-001",
		ExportID:       "mikro-export-001",
		TargetSystem:   TargetSystemMikro,
		FormatVersion:  MikroFormatV1,
		PeriodCode:     "2026-05",
		FiscalYear:     2026,
		Postings:       []postingruntime.PostingEntry{validPosting()},
		RequestedBy:    "mikro-export-test",
...
func newRuntime(t *testing.T) *MikroRealFormatRuntime {
	t.Helper()

	runtime, err := NewMikroRealFormatRuntime(validConfig())
	if err != nil {
...
func TestBuildMikroPackageReady(t *testing.T) {
	runtime := newRuntime(t)

	pkg, err := runtime.BuildPackage(validRequest())
	if err != nil {
		t.Fatalf("build Mikro package failed: %v", err)
	}
...
func TestMikroJournalFileContainsExpectedRows(t *testing.T) {
	runtime := newRuntime(t)
	...
	if !strings.Contains(journal, "FIS_TARIHI;FIS_NO;SATIR_NO;EVRAK_NO;HESAP_KODU") {
		t.Fatal("expected Mikro journal header")
	}
...
func TestMikroLedgerAndSummaryFilesGenerated(t *testing.T) {
	runtime := newRuntime(t)
	...
	for _, file := range pkg.Files {
		if file.FileType == FileTypeMikroLedgerCSV {
			ledgerFound = true
			if !strings.Contains(file.Content, "HESAP_KODU;HESAP_ADI;BORC;ALACAK;DOVIZ") {
				t.Fatal("expected Mikro ledger header")
			}
		}
		if file.FileType == FileTypeMikroSummaryTXT {
			summaryFound = true
			if !strings.Contains(file.Content, "BALANCED;true") {
				t.Fatal("expected balanced Mikro summary")
			}
		}
	}

	if !ledgerFound {
		t.Fatal("expected Mikro ledger file")
	}
	if !summaryFound {
		t.Fatal("expected Mikro summary file")
	}
}
...
func TestNormalizeTurkishCharacters(t *testing.T) {
	got := normalizeMikroField("Çalışma Şirketi İğdır Ürün", true, 100)
...
func TestRuntimeRejectsDisabledConfig(t *testing.T) {
	...
	_, err := NewMikroRealFormatRuntime(cfg)
	if err == nil {
