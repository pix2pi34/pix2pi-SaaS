...
func TestNormalizePort(t *testing.T) {
	if got := normalizePort(":5930"); got != "5930" {
		t.Fatalf("beklenen 5930, gelen %s", got)
	}

	if got := normalizePort(""); got != "5930" {
		t.Fatalf("bos port default 5930 olmali, gelen %s", got)
	}
}
...
func TestLoadConfig_DefaultPort(t *testing.T) {
	t.Setenv("NOTIFICATION_RUNTIME_PORT", "")
	t.Setenv("DB_READ_DSN", "read-dsn")

	cfg := loadConfig()

	if cfg.Port != "5930" {
		t.Fatalf("beklenen port 5930, gelen %s", cfg.Port)
	}
...
func TestLoadConfig_WriteFallback(t *testing.T) {
	t.Setenv("NOTIFICATION_RUNTIME_PORT", "5994")
	...
	if cfg.Port != "5994" {
		t.Fatalf("beklenen port 5994, gelen %s", cfg.Port)
	}
