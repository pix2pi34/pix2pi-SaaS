...
func TestNormalizePort(t *testing.T) {
	if got := normalizePort(":5880"); got != "5880" {
		t.Fatalf("beklenen 5880, gelen %s", got)
	}

	if got := normalizePort(""); got != "5880" {
		t.Fatalf("bos port default 5880 olmali, gelen %s", got)
	}
}
...
func TestLoadConfig_DefaultPort(t *testing.T) {
	t.Setenv("JOBS_RUNTIME_PORT", "")
	t.Setenv("DB_READ_DSN", "read-dsn")

	cfg := loadConfig()

	if cfg.Port != "5880" {
		t.Fatalf("beklenen port 5880, gelen %s", cfg.Port)
	}
...
func TestLoadConfig_WriteFallback(t *testing.T) {
	t.Setenv("JOBS_RUNTIME_PORT", "5999")
	...
	if cfg.Port != "5999" {
		t.Fatalf("beklenen port 5999, gelen %s", cfg.Port)
	}
