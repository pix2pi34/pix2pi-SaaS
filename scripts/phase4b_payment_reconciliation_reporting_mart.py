...
migration_base = os.environ.get("MIGRATION_BASE", "20260428_154001_payment_reconciliation_reporting_mart").strip()

report_dir = root / "docs/phase4"
migration_dir = root / "db/migrations"

standard_file = report_dir / "15_4_payment_reconciliation_reporting_mart_standard.md"
report_file = report_dir / "15_4_payment_reconciliation_reporting_mart_report.md"
inventory_file = report_dir / "15_4_payment_reconciliation_reporting_mart_inventory.tsv"
matrix_file = report_dir / "15_4_payment_reconciliation_reporting_mart_matrix.tsv"
...
detail("VALIDATION_MODE=PAYMENT_RECONCILIATION_REPORTING_MART_MIGRATION_PAIR_ONLY")
...
prev_15_3_db_apply = get_value(prev_15_3, "DB_APPLY_EXECUTED")
...
detail(f"PREVIOUS_15_3_DB_APPLY_EXECUTED={prev_15_3_db_apply}")
...
if prev_15_3_db_apply != "NO":
    fail("15.3 DB apply NO degil")

if not standard_file.exists():
    fail("15.4 standard doc yok")
...
summary_date_count = count(r"\bsummary_date\s+date\s+NOT\s+NULL\b", up_text)
settlement_date_count = count(r"\bsettlement_date\s+date\s+NOT\s+NULL\b", up_text)
payout_date_count = count(r"\bpayout_date\s+date\s+NOT\s+NULL\b", up_text)
provider_code_count = count(r"\bprovider_code\s+text\s+NOT\s+NULL", up_text)
...
rate_metric_count = count(r"\bnumeric\(9,4\)", up_text)
...
detail(f"PAYMENT_RECONCILIATION_TABLE_COUNT={create_table_count}")
detail(f"PAYMENT_RECONCILIATION_EXPECTED_TABLE_MISSING_COUNT={len(missing_tables)}")
detail(f"PAYMENT_RECONCILIATION_TENANT_ID_COLUMN_COUNT={tenant_id_column_count}")
detail(f"PAYMENT_RECONCILIATION_PERIOD_KEY_COLUMN_COUNT={period_key_count}")
detail(f"PAYMENT_RECONCILIATION_SUMMARY_DATE_COLUMN_COUNT={summary_date_count}")
detail(f"PAYMENT_RECONCILIATION_SETTLEMENT_DATE_COLUMN_COUNT={settlement_date_count}")
detail(f"PAYMENT_RECONCILIATION_PAYOUT_DATE_COLUMN_COUNT={payout_date_count}")
detail(f"PAYMENT_RECONCILIATION_PROVIDER_CODE_COLUMN_COUNT={provider_code_count}")
detail(f"PAYMENT_RECONCILIATION_STATUS_CODE_COLUMN_COUNT={status_code_count}")
detail(f"PAYMENT_RECONCILIATION_CURRENCY_CODE_COLUMN_COUNT={currency_code_count}")
detail(f"PAYMENT_RECONCILIATION_NUMERIC_AMOUNT_COLUMN_COUNT={numeric_amount_count}")
detail(f"PAYMENT_RECONCILIATION_RATE_METRIC_COUNT={rate_metric_count}")
detail(f"PAYMENT_RECONCILIATION_UNIQUE_CONSTRAINT_COUNT={unique_constraint_count}")
detail(f"PAYMENT_RECONCILIATION_INDEX_COUNT={create_index_count}")
detail(f"PAYMENT_RECONCILIATION_DOWN_DROP_TABLE_COUNT={drop_table_count}")
...
if create_table_count != len(expected_tables):
    fail("expected payment/reconciliation reporting table count 7 degil")
if missing_tables:
    fail("expected payment/reconciliation reporting table eksik: " + ",".join(missing_tables))
if tenant_id_column_count != len(expected_tables):
    fail("tenant_id not null column count 7 degil")
if period_key_count != len(expected_tables):
    fail("period_key count 7 degil")
if summary_date_count < 1:
    fail("summary_date count 1 altinda")
if settlement_date_count < 1:
    fail("settlement_date count 1 altinda")
if payout_date_count < 1:
    fail("payout_date count 1 altinda")
if provider_code_count < 7:
    fail("provider_code count 7 altinda")
if status_code_count < 7:
    fail("status_code count 7 altinda")
if currency_code_count != len(expected_tables):
    fail("currency_code count 7 degil")
if numeric_amount_count < 25:
    fail("numeric amount column count 25 altinda")
if rate_metric_count < 2:
    fail("rate metric count 2 altinda")
...
period_status = "PASS" if period_key_count == 7 and summary_date_count >= 1 and settlement_date_count >= 1 and payout_date_count >= 1 else "FAIL"
provider_status = "PASS" if provider_code_count >= 7 else "FAIL"
status_status = "PASS" if status_code_count >= 7 else "FAIL"
currency_status = "PASS" if currency_code_count == 7 else "FAIL"
amount_status = "PASS" if numeric_amount_count >= 25 and rate_metric_count >= 2 else "FAIL"
...
detail(f"PAYMENT_RECONCILIATION_MIGRATION_PAIR={pair_status}")
detail(f"PAYMENT_RECONCILIATION_SCHEMA_STATUS={schema_status}")
detail(f"PAYMENT_RECONCILIATION_TABLE_STATUS={table_status}")
detail(f"PAYMENT_RECONCILIATION_TENANT_SAFETY_STATUS={tenant_status}")
detail(f"PAYMENT_RECONCILIATION_PERIOD_STATUS={period_status}")
detail(f"PAYMENT_RECONCILIATION_PROVIDER_STATUS={provider_status}")
detail(f"PAYMENT_RECONCILIATION_STATUS_CODE_STATUS={status_status}")
detail(f"PAYMENT_RECONCILIATION_CURRENCY_STATUS={currency_status}")
detail(f"PAYMENT_RECONCILIATION_AMOUNT_STATUS={amount_status}")
detail(f"PAYMENT_RECONCILIATION_INDEX_STATUS={index_status}")
detail(f"PAYMENT_RECONCILIATION_DOWN_STATUS={down_status}")
detail(f"PAYMENT_RECONCILIATION_RISK_STATUS={risk_status}")
detail(f"PAYMENT_RECONCILIATION_CHAIN_STATUS={chain_status}")
...
inventory_lines = ["table\tstatus\ttenant_id_required\tperiod_key\tdate_key\tprovider_code\tstatus_code\tcurrency_code\tnote"]
date_key_by_table = {
    "payment_attempt_summaries": "summary_date",
    "payment_provider_summaries": "period_only",
    "settlement_summaries": "settlement_date",
    "reconciliation_difference_summaries": "period_only",
    "commission_summaries": "period_only",
    "merchant_payout_summaries": "payout_date",
    "payment_reconciliation_tenant_kpis": "period_only",
}
for table in expected_tables:
    table_present = "YES" if table in unique_tables else "NO"
    inventory_lines.append(
        f"{table}\t{table_present}\tYES\tYES\t{date_key_by_table[table]}\tYES\tYES\tYES\treporting_mart.{table}"
    )
...
matrix_lines = [
    ...
    f"period_date\t{period_status}\tperiod={period_key_count} summary={summary_date_count} settlement={settlement_date_count} payout={payout_date_count}",
    f"provider\t{provider_status}\tprovider_code={provider_code_count}",
    f"status_code\t{status_status}\tstatus_code={status_code_count}",
    f"currency\t{currency_status}\tcurrency_code={currency_code_count}",
    f"amount_rate\t{amount_status}\tnumeric={numeric_amount_count} rate={rate_metric_count}",
...
detail(f"PAYMENT_RECONCILIATION_INVENTORY_LINE_COUNT={len(inventory_lines)}")
detail(f"PAYMENT_RECONCILIATION_MATRIX_LINE_COUNT={len(matrix_lines)}")

final_status = "PASS" if not failures else "FAIL"
detail(f"PAYMENT_RECONCILIATION_REPORTING_MART={final_status}")
detail(f"FAZ4B_15_4_FINAL_STATUS={final_status}")
...
report_lines = [
    ...
    *details,
    ...
    f"PAYMENT_RECONCILIATION_REPORTING_MART={final_status}",
    f"FAZ4B_15_4_FINAL_STATUS={final_status}",
    "",
    "## Tool Status",
    *tools,
    "",
...
print(f"PAYMENT_RECONCILIATION_TABLE_COUNT={create_table_count}")
print(f"PAYMENT_RECONCILIATION_TENANT_ID_COLUMN_COUNT={tenant_id_column_count}")
print(f"PAYMENT_RECONCILIATION_PERIOD_KEY_COLUMN_COUNT={period_key_count}")
print(f"PAYMENT_RECONCILIATION_INDEX_COUNT={create_index_count}")
print(f"PAYMENT_RECONCILIATION_MIGRATION_PAIR={pair_status}")
print(f"PAYMENT_RECONCILIATION_TABLE_STATUS={table_status}")
print(f"PAYMENT_RECONCILIATION_TENANT_SAFETY_STATUS={tenant_status}")
print(f"PAYMENT_RECONCILIATION_CHAIN_STATUS={chain_status}")
print("DB_MUTATION=NO")
print("DB_APPLY_EXECUTED=NO")
print("MIGRATION_APPLY_EXECUTED=NO")
print("QUERY_TEXT_PRINTED=NO")
print(f"PAYMENT_RECONCILIATION_REPORTING_MART={final_status} {'✅' if final_status == 'PASS' else '❌'}")
