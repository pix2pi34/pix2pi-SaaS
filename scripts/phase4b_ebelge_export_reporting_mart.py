...
migration_base = os.environ.get("MIGRATION_BASE", "20260428_153001_ebelge_export_reporting_mart").strip()

report_dir = root / "docs/phase4"
migration_dir = root / "db/migrations"

standard_file = report_dir / "15_3_ebelge_export_reporting_mart_standard.md"
report_file = report_dir / "15_3_ebelge_export_reporting_mart_report.md"
inventory_file = report_dir / "15_3_ebelge_export_reporting_mart_inventory.tsv"
matrix_file = report_dir / "15_3_ebelge_export_reporting_mart_matrix.tsv"
...
detail("VALIDATION_MODE=EBELGE_EXPORT_REPORTING_MART_MIGRATION_PAIR_ONLY")
...
prev_15_2_db_apply = get_value(prev_15_2, "DB_APPLY_EXECUTED")
...
detail(f"PREVIOUS_15_2_DB_APPLY_EXECUTED={prev_15_2_db_apply}")
...
if prev_15_2_db_apply != "NO":
    fail("15.2 DB apply NO degil")

if not standard_file.exists():
    fail("15.3 standard doc yok")
...
ebelge_type_count = count(r"\bebelge_type\s+text\s+NOT\s+NULL\b", up_text)
...
detail(f"EBELGE_EXPORT_TABLE_COUNT={create_table_count}")
detail(f"EBELGE_EXPORT_EXPECTED_TABLE_MISSING_COUNT={len(missing_tables)}")
detail(f"EBELGE_EXPORT_TENANT_ID_COLUMN_COUNT={tenant_id_column_count}")
detail(f"EBELGE_EXPORT_PERIOD_KEY_COLUMN_COUNT={period_key_count}")
detail(f"EBELGE_EXPORT_SUMMARY_DATE_COLUMN_COUNT={summary_date_count}")
detail(f"EBELGE_EXPORT_EBELGE_TYPE_COLUMN_COUNT={ebelge_type_count}")
detail(f"EBELGE_EXPORT_PROVIDER_CODE_COLUMN_COUNT={provider_code_count}")
detail(f"EBELGE_EXPORT_STATUS_CODE_COLUMN_COUNT={status_code_count}")
detail(f"EBELGE_EXPORT_NUMERIC_AMOUNT_COLUMN_COUNT={numeric_amount_count}")
detail(f"EBELGE_EXPORT_UNIQUE_CONSTRAINT_COUNT={unique_constraint_count}")
detail(f"EBELGE_EXPORT_INDEX_COUNT={create_index_count}")
detail(f"EBELGE_EXPORT_DOWN_DROP_TABLE_COUNT={drop_table_count}")
...
if create_table_count != len(expected_tables):
    fail("expected ebelge/export reporting table count 7 degil")
if missing_tables:
    fail("expected ebelge/export reporting table eksik: " + ",".join(missing_tables))
...
if ebelge_type_count < 3:
    fail("ebelge_type count 3 altinda")
if provider_code_count < 4:
    fail("provider_code count 4 altinda")
...
period_status = "PASS" if period_key_count == 7 and summary_date_count >= 1 else "FAIL"
ebelge_status = "PASS" if ebelge_type_count >= 3 else "FAIL"
provider_status = "PASS" if provider_code_count >= 4 else "FAIL"
...
detail(f"EBELGE_EXPORT_MIGRATION_PAIR={pair_status}")
detail(f"EBELGE_EXPORT_SCHEMA_STATUS={schema_status}")
detail(f"EBELGE_EXPORT_TABLE_STATUS={table_status}")
detail(f"EBELGE_EXPORT_TENANT_SAFETY_STATUS={tenant_status}")
detail(f"EBELGE_EXPORT_PERIOD_STATUS={period_status}")
detail(f"EBELGE_EXPORT_EBELGE_TYPE_STATUS={ebelge_status}")
detail(f"EBELGE_EXPORT_PROVIDER_STATUS={provider_status}")
detail(f"EBELGE_EXPORT_INDEX_STATUS={index_status}")
detail(f"EBELGE_EXPORT_DOWN_STATUS={down_status}")
detail(f"EBELGE_EXPORT_RISK_STATUS={risk_status}")
detail(f"EBELGE_EXPORT_CHAIN_STATUS={chain_status}")
...
inventory_lines = ["table\tstatus\ttenant_id_required\tperiod_key\tsummary_date\ttype_or_provider\tnote"]
for table in expected_tables:
    table_present = "YES" if table in unique_tables else "NO"
    summary = "YES" if table == "ebelge_daily_summaries" else "NO"
    if table.startswith("ebelge_"):
        type_or_provider = "ebelge_type"
    else:
        type_or_provider = "provider_code"
    inventory_lines.append(
        f"{table}\t{table_present}\tYES\tYES\t{summary}\t{type_or_provider}\treporting_mart.{table}"
    )
...
matrix_lines = [
    ...
    f"period_date\t{period_status}\tperiod={period_key_count} summary_date={summary_date_count}",
    f"ebelge_type\t{ebelge_status}\tebelge_type={ebelge_type_count}",
...
detail(f"EBELGE_EXPORT_INVENTORY_LINE_COUNT={len(inventory_lines)}")
detail(f"EBELGE_EXPORT_MATRIX_LINE_COUNT={len(matrix_lines)}")

final_status = "PASS" if not failures else "FAIL"
detail(f"EBELGE_EXPORT_REPORTING_MART={final_status}")
detail(f"FAZ4B_15_3_FINAL_STATUS={final_status}")
...
report_lines = [
    ...
    *details,
    ...
    f"EBELGE_EXPORT_REPORTING_MART={final_status}",
    f"FAZ4B_15_3_FINAL_STATUS={final_status}",
    "",
    "## Tool Status",
    *tools,
    "",
...
print(f"EBELGE_EXPORT_TABLE_COUNT={create_table_count}")
print(f"EBELGE_EXPORT_TENANT_ID_COLUMN_COUNT={tenant_id_column_count}")
print(f"EBELGE_EXPORT_PERIOD_KEY_COLUMN_COUNT={period_key_count}")
print(f"EBELGE_EXPORT_INDEX_COUNT={create_index_count}")
print(f"EBELGE_EXPORT_MIGRATION_PAIR={pair_status}")
print(f"EBELGE_EXPORT_TABLE_STATUS={table_status}")
print(f"EBELGE_EXPORT_TENANT_SAFETY_STATUS={tenant_status}")
print(f"EBELGE_EXPORT_CHAIN_STATUS={chain_status}")
print("DB_MUTATION=NO")
print("DB_APPLY_EXECUTED=NO")
print("MIGRATION_APPLY_EXECUTED=NO")
print("QUERY_TEXT_PRINTED=NO")
print(f"EBELGE_EXPORT_REPORTING_MART={final_status} {'✅' if final_status == 'PASS' else '❌'}")
