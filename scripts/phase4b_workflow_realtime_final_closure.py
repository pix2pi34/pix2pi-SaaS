...
standard_file = report_dir / "17_7_workflow_realtime_final_closure_standard.md"
report_file = report_dir / "17_7_workflow_realtime_final_closure_report.md"
matrix_file = report_dir / "17_7_workflow_realtime_final_closure_matrix.tsv"
inventory_file = report_dir / "17_7_workflow_realtime_final_closure_inventory.tsv"
closure_file = report_dir / "17_workflow_realtime_ui_final_closure_report.md"
...
reports = {
    ...
    "17.6": report_dir / "17_6_workflow_realtime_tests_report.md",
...
required_pass_keys = {
    ...
    "17.6": [
        "WORKFLOW_TEST_BASELINE",
        "WORKFLOW_TEST_STATE_MACHINE",
        "WORKFLOW_TEST_ACTION_APPROVAL",
        "WORKFLOW_TEST_REALTIME_CHANNEL",
        "WORKFLOW_TEST_UI_API_PLAN",
        "WORKFLOW_TEST_ARTIFACT_COVERAGE",
        "WORKFLOW_TEST_NO_RUNTIME_CHANGE",
        "WORKFLOW_TEST_NO_CONFIG_CHANGE",
        "WORKFLOW_TEST_SECRET_SAFE",
    ],
}
...
metric_keys = {
    ...
    "17.6": [
        "WORKFLOW_TEST_METRIC_EVIDENCE_TOTAL",
        "WORKFLOW_TEST_GATE_FAILURE_COUNT",
        "WORKFLOW_TEST_NO_CHANGE_FAILURE_COUNT",
        "WORKFLOW_TEST_ARTIFACT_MISSING_COUNT",
        "WORKFLOW_TEST_METRIC_MISSING_COUNT",
    ],
}
...
artifact_sets = {
    ...
    "17.6": [
        "docs/phase4/17_6_workflow_realtime_tests_standard.md",
        "docs/phase4/17_6_workflow_realtime_tests_report.md",
        "docs/phase4/17_6_workflow_realtime_tests_matrix.tsv",
        "docs/phase4/17_6_workflow_realtime_tests_inventory.tsv",
...
detail("VALIDATION_MODE=WORKFLOW_REALTIME_FINAL_CLOSURE_ONLY")

for tool in ["python3", "grep", "wc"]:
    tool_status(tool)

if not standard_file.exists():
    fail("17.7 standard doc yok")
...
for block in ["17.1", "17.2", "17.3", "17.4", "17.5", "17.6"]:
    report = reports[block]
    final_status = get_value(report, final_keys[block])
    domain_status = get_value(report, domain_keys[block])

    detail(f"WORKFLOW_FINAL_{block.replace('.', '_')}_FINAL_STATUS={final_status}")
    detail(f"WORKFLOW_FINAL_{block.replace('.', '_')}_{domain_keys[block]}={domain_status}")
    ...
    for key in required_pass_keys[block]:
        ...
        detail(f"WORKFLOW_FINAL_{block.replace('.', '_')}_{key}={value}")
        if value != "PASS":
    ...
    detail(f"WORKFLOW_FINAL_{block.replace('.', '_')}_ARTIFACT_EXPECTED_COUNT={expected_artifact_count}")
    detail(f"WORKFLOW_FINAL_{block.replace('.', '_')}_ARTIFACT_EXISTING_COUNT={existing_artifact_count}")
...
if gate_failures:
    fail("workflow realtime final gate failure: " + ",".join(gate_failures[:80]))

if no_change_failures:
    fail("workflow realtime final no-change failure: " + ",".join(no_change_failures[:80]))

if artifact_missing:
    fail("workflow realtime final artifact eksik: " + ",".join(artifact_missing[:80]))

if metric_missing:
    fail("workflow realtime final metric evidence eksik: " + ",".join(metric_missing[:80]))
...
tests_status = block_results["17.6"]["status"]
...
for label, status in [
    ("WORKFLOW_FINAL_BASELINE", baseline_status),
    ("WORKFLOW_FINAL_STATE_MACHINE", state_machine_status),
    ("WORKFLOW_FINAL_ACTION_APPROVAL", action_approval_status),
    ("WORKFLOW_FINAL_REALTIME_CHANNEL", realtime_channel_status),
    ("WORKFLOW_FINAL_UI_API_PLAN", ui_api_plan_status),
    ("WORKFLOW_FINAL_TESTS", tests_status),
    ("WORKFLOW_FINAL_ARTIFACT_COVERAGE", artifact_coverage_status),
    ("WORKFLOW_FINAL_NO_RUNTIME_CHANGE", no_runtime_change_status),
    ("WORKFLOW_FINAL_NO_CONFIG_CHANGE", no_config_change_status),
    ("WORKFLOW_FINAL_SECRET_SAFE", secret_safe_status),
]:
    detail(f"{label}={status}")
    if status != "PASS":
        fail(f"{label} PASS degil")

detail(f"WORKFLOW_FINAL_METRIC_EVIDENCE_TOTAL={metric_total}")
detail(f"WORKFLOW_FINAL_GATE_FAILURE_COUNT={len(gate_failures)}")
detail(f"WORKFLOW_FINAL_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
detail(f"WORKFLOW_FINAL_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
detail(f"WORKFLOW_FINAL_METRIC_MISSING_COUNT={len(metric_missing)}")

matrix_lines = [
    "gate\tstatus\tnote",
    f"baseline\t{baseline_status}\t17.1 workflow realtime baseline final",
    f"state_machine\t{state_machine_status}\t17.2 workflow state machine final",
    f"action_approval\t{action_approval_status}\t17.3 action approval final",
    f"realtime_channel\t{realtime_channel_status}\t17.4 realtime channel final",
    f"ui_api_plan\t{ui_api_plan_status}\t17.5 UI/API plan final",
    f"workflow_tests\t{tests_status}\t17.6 workflow realtime tests final",
    ...
    "frontend_file_created\tNO\tfinal evidence only",
    "api_route_created\tNO\tfinal evidence only",
    "api_implementation_changed\tNO\tfinal evidence only",
    "dto_code_created\tNO\tfinal evidence only",
    "handler_code_created\tNO\tfinal evidence only",
    "middleware_changed\tNO\tfinal evidence only",
    "websocket_server_started\tNO\tfinal evidence only",
    "sse_server_started\tNO\tfinal evidence only",
    "realtime_runtime_changed\tNO\tfinal evidence only",
    "workflow_runtime_changed\tNO\tfinal evidence only",
    "approval_runtime_changed\tNO\tfinal evidence only",
...
detail(f"WORKFLOW_FINAL_MATRIX_LINE_COUNT={len(matrix_lines)}")
detail(f"WORKFLOW_FINAL_INVENTORY_LINE_COUNT={len(inventory_lines)}")

final_status = "PASS" if not failures else "FAIL"
detail(f"WORKFLOW_REALTIME_FINAL_CLOSURE={final_status}")
detail(f"FAZ4B_17_7_FINAL_STATUS={final_status}")
detail(f"FAZ4B_17_FINAL_STATUS={final_status}")

report_lines = [
    "# FAZ 4B / 17.7 - Workflow / Realtime UI Final Closure Report",
    "",
    f"Generated at: {now()}",
    "",
    "## Summary",
    *details,
    ...
    f"WORKFLOW_REALTIME_FINAL_CLOSURE={final_status}",
    f"FAZ4B_17_7_FINAL_STATUS={final_status}",
    f"FAZ4B_17_FINAL_STATUS={final_status}",
    "",
    "## Tool Status",
    *tools,
    "",
    "## Matrix",
    "MATRIX_FILE=docs/phase4/17_7_workflow_realtime_final_closure_matrix.tsv",
    matrix_file.read_text(errors="ignore").rstrip(),
    "",
    "## Inventory",
    "INVENTORY_FILE=docs/phase4/17_7_workflow_realtime_final_closure_inventory.tsv",
...
closure_lines = [
    ...
    "",
    "## Closed Items",
    f"17.1 Workflow / Realtime UI baseline / surface inventory={baseline_status}",
    f"17.2 Workflow state machine contract={state_machine_status}",
    f"17.3 Workflow action / approval contract={action_approval_status}",
    f"17.4 Realtime channel contract={realtime_channel_status}",
    f"17.5 UI surface + API implementation plan={ui_api_plan_status}",
    f"17.6 Workflow / realtime tests={tests_status}",
    f"17.7 Workflow / Realtime UI final closure={final_status}",
    "",
    "## Final Gates",
    f"WORKFLOW_FINAL_BASELINE={baseline_status}",
    f"WORKFLOW_FINAL_STATE_MACHINE={state_machine_status}",
    f"WORKFLOW_FINAL_ACTION_APPROVAL={action_approval_status}",
    f"WORKFLOW_FINAL_REALTIME_CHANNEL={realtime_channel_status}",
    f"WORKFLOW_FINAL_UI_API_PLAN={ui_api_plan_status}",
    f"WORKFLOW_FINAL_TESTS={tests_status}",
    f"WORKFLOW_FINAL_ARTIFACT_COVERAGE={artifact_coverage_status}",
    f"WORKFLOW_FINAL_NO_RUNTIME_CHANGE={no_runtime_change_status}",
    f"WORKFLOW_FINAL_NO_CONFIG_CHANGE={no_config_change_status}",
    f"WORKFLOW_FINAL_SECRET_SAFE={secret_safe_status}",
    f"WORKFLOW_FINAL_METRIC_EVIDENCE_TOTAL={metric_total}",
    "",
...
print(f"WORKFLOW_FINAL_BASELINE={baseline_status}")
print(f"WORKFLOW_FINAL_STATE_MACHINE={state_machine_status}")
print(f"WORKFLOW_FINAL_ACTION_APPROVAL={action_approval_status}")
print(f"WORKFLOW_FINAL_REALTIME_CHANNEL={realtime_channel_status}")
print(f"WORKFLOW_FINAL_UI_API_PLAN={ui_api_plan_status}")
print(f"WORKFLOW_FINAL_TESTS={tests_status}")
print(f"WORKFLOW_FINAL_ARTIFACT_COVERAGE={artifact_coverage_status}")
print(f"WORKFLOW_FINAL_NO_RUNTIME_CHANGE={no_runtime_change_status}")
print(f"WORKFLOW_FINAL_NO_CONFIG_CHANGE={no_config_change_status}")
print(f"WORKFLOW_FINAL_SECRET_SAFE={secret_safe_status}")
print(f"WORKFLOW_FINAL_METRIC_EVIDENCE_TOTAL={metric_total}")
print(f"WORKFLOW_FINAL_GATE_FAILURE_COUNT={len(gate_failures)}")
print(f"WORKFLOW_FINAL_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
print(f"WORKFLOW_FINAL_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
print(f"WORKFLOW_FINAL_METRIC_MISSING_COUNT={len(metric_missing)}")
...
print(f"WORKFLOW_REALTIME_FINAL_CLOSURE={final_status} {'✅' if final_status == 'PASS' else '❌'}")
print(f"FAZ4B_17_7_FINAL_STATUS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
print(f"FAZ4B_17_FINAL_STATUS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
