...
standard_file = report_dir / "20_7_production_hardening_tests_standard.md"
report_file = report_dir / "20_7_production_hardening_tests_report.md"
matrix_file = report_dir / "20_7_production_hardening_tests_matrix.tsv"
inventory_file = report_dir / "20_7_production_hardening_tests_inventory.tsv"
...
detail("VALIDATION_MODE=PRODUCTION_HARDENING_TESTS_EVIDENCE_ONLY")

for tool in ["python3", "grep", "wc"]:
    tool_status(tool)

if not standard_file.exists():
    fail("20.7 standard doc yok")
...
for block in ["20.1", "20.2", "20.3", "20.4", "20.5", "20.6"]:
    ...
    for key in required_pass_keys[block]:
        ...
        detail(f"PRODUCTION_TEST_{block.replace('.', '_')}_{key}={value}")
        if value != "PASS":
    ...
    detail(f"PRODUCTION_TEST_{block.replace('.', '_')}_ARTIFACT_EXPECTED_COUNT={expected_artifact_count}")
    detail(f"PRODUCTION_TEST_{block.replace('.', '_')}_ARTIFACT_EXISTING_COUNT={existing_artifact_count}")

    if local_failures:
        status_failures.extend([f"{block}:{x}" for x in local_failures])
    ...
    block_results[block] = {
        ...
        "status_failures": len(local_failures),
    }

if status_failures:
    fail("production gate failure: " + ",".join(status_failures[:40]))

if no_change_failures:
    fail("production no-change failure: " + ",".join(no_change_failures[:40]))

if artifact_missing:
    fail("production artifact eksik: " + ",".join(artifact_missing[:40]))

if risk_evidence_missing:
    fail("production risk evidence eksik: " + ",".join(risk_evidence_missing[:40]))

cleanup_status = block_results["20.1"]["status"]
config_env_status = block_results["20.2"]["status"]
runtime_status = block_results["20.3"]["status"]
nginx_status = block_results["20.4"]["status"]
docker_status = block_results["20.5"]["status"]
backup_status = block_results["20.6"]["status"]
...
for label, status in [
    ("PRODUCTION_TEST_CLEANUP", cleanup_status),
    ("PRODUCTION_TEST_CONFIG_ENV", config_env_status),
    ("PRODUCTION_TEST_RUNTIME_SERVICE", runtime_status),
    ("PRODUCTION_TEST_NGINX", nginx_status),
    ("PRODUCTION_TEST_DOCKER", docker_status),
    ("PRODUCTION_TEST_BACKUP_ARCHIVE", backup_status),
    ("PRODUCTION_TEST_ARTIFACT_COVERAGE", artifact_coverage_status),
    ("PRODUCTION_TEST_NO_CHANGE", no_change_status),
    ("PRODUCTION_TEST_RISK_EVIDENCE", risk_evidence_status),
    ("PRODUCTION_TEST_SECRET_SAFE", secret_safe_status),
]:
    detail(f"{label}={status}")
    if status != "PASS":
        fail(f"{label} PASS degil")

detail(f"PRODUCTION_TEST_RISK_EVIDENCE_TOTAL={risk_total}")
detail(f"PRODUCTION_TEST_STATUS_FAILURE_COUNT={len(status_failures)}")
detail(f"PRODUCTION_TEST_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
detail(f"PRODUCTION_TEST_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
detail(f"PRODUCTION_TEST_RISK_EVIDENCE_MISSING_COUNT={len(risk_evidence_missing)}")

matrix_lines = [
    "gate\tstatus\tnote",
    f"cleanup\t{cleanup_status}\t20.1 production cleanup",
    f"config_env\t{config_env_status}\t20.2 config/env hardening",
    f"runtime_service\t{runtime_status}\t20.3 runtime service hardening",
    f"nginx_reverse_proxy\t{nginx_status}\t20.4 nginx/reverse proxy hardening",
    f"docker_compose\t{docker_status}\t20.5 docker/compose hardening",
    f"backup_archive\t{backup_status}\t20.6 backup/archive retention",
...
for block in ["20.1", "20.2", "20.3", "20.4", "20.5", "20.6"]:
    ...
    inventory_lines.append(
        f"{block}\t{result['status']}\t{domain_keys[block]}\t{str(reports[block].relative_to(root))}\t{result['artifact_expected']}\t{result['artifact_existing']}\t{result['risk_key_hits']}\t{result['risk_key_expected']}\t{result['risk_key_sum']}\t{result['status_failures']}"
    )

inventory_file.write_text("\n".join(inventory_lines) + "\n")

detail(f"PRODUCTION_TEST_MATRIX_LINE_COUNT={len(matrix_lines)}")
detail(f"PRODUCTION_TEST_INVENTORY_LINE_COUNT={len(inventory_lines)}")

final_status = "PASS" if not failures else "FAIL"
detail(f"PRODUCTION_HARDENING_TESTS={final_status}")
detail(f"FAZ4B_20_7_FINAL_STATUS={final_status}")
...
report_lines = [
    ...
    *details,
    ...
    f"PRODUCTION_HARDENING_TESTS={final_status}",
    f"FAZ4B_20_7_FINAL_STATUS={final_status}",
    "",
    "## Tool Status",
    *tools,
    "",
...
print(f"PRODUCTION_TEST_CLEANUP={cleanup_status}")
print(f"PRODUCTION_TEST_CONFIG_ENV={config_env_status}")
print(f"PRODUCTION_TEST_RUNTIME_SERVICE={runtime_status}")
print(f"PRODUCTION_TEST_NGINX={nginx_status}")
print(f"PRODUCTION_TEST_DOCKER={docker_status}")
print(f"PRODUCTION_TEST_BACKUP_ARCHIVE={backup_status}")
print(f"PRODUCTION_TEST_ARTIFACT_COVERAGE={artifact_coverage_status}")
print(f"PRODUCTION_TEST_NO_CHANGE={no_change_status}")
print(f"PRODUCTION_TEST_RISK_EVIDENCE={risk_evidence_status}")
print(f"PRODUCTION_TEST_SECRET_SAFE={secret_safe_status}")
print(f"PRODUCTION_TEST_RISK_EVIDENCE_TOTAL={risk_total}")
print(f"PRODUCTION_TEST_STATUS_FAILURE_COUNT={len(status_failures)}")
print(f"PRODUCTION_TEST_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
print(f"PRODUCTION_TEST_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
...
print(f"PRODUCTION_HARDENING_TESTS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
print(f"FAZ4B_20_7_FINAL_STATUS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
