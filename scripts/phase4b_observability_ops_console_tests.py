...
standard_file = report_dir / "22_7_observability_ops_console_tests_standard.md"
report_file = report_dir / "22_7_observability_ops_console_tests_report.md"
matrix_file = report_dir / "22_7_observability_ops_console_tests_matrix.tsv"
inventory_file = report_dir / "22_7_observability_ops_console_tests_inventory.tsv"
...
detail("VALIDATION_MODE=OBSERVABILITY_OPS_CONSOLE_TESTS_ONLY")

for tool in ["python3", "grep", "wc"]:
    tool_status(tool)

if not standard_file.exists():
    fail("22.7 standard doc yok")
...
for block in ["22.1", "22.2", "22.3", "22.4", "22.5", "22.6"]:
    report = reports[block]

    final_status = get_value(report, final_keys[block])
    domain_status = get_value(report, domain_keys[block])

    detail(f"OBS_TEST_{block.replace('.', '_')}_FINAL_STATUS={final_status}")
    detail(f"OBS_TEST_{block.replace('.', '_')}_{domain_keys[block]}={domain_status}")
    ...
    for key in required_pass_keys[block]:
        ...
        detail(f"OBS_TEST_{block.replace('.', '_')}_{key}={value}")
        if value != "PASS":
    ...
    detail(f"OBS_TEST_{block.replace('.', '_')}_ARTIFACT_EXPECTED_COUNT={expected_artifact_count}")
    detail(f"OBS_TEST_{block.replace('.', '_')}_ARTIFACT_EXISTING_COUNT={existing_artifact_count}")
...
if gate_failures:
    fail("observability gate failure: " + ",".join(gate_failures[:60]))

if no_change_failures:
    fail("observability no-change failure: " + ",".join(no_change_failures[:60]))

if artifact_missing:
    fail("observability artifact eksik: " + ",".join(artifact_missing[:60]))

if metric_missing:
    fail("observability metric evidence eksik: " + ",".join(metric_missing[:60]))
...
for label, status in [
    ("OBS_TEST_BASELINE", baseline_status),
    ("OBS_TEST_METRICS", metrics_status),
    ("OBS_TEST_LOGS", logs_status),
    ("OBS_TEST_TRACES", traces_status),
    ("OBS_TEST_ALERTS", alerts_status),
    ("OBS_TEST_OPS_CONSOLE", ops_status),
    ("OBS_TEST_ARTIFACT_COVERAGE", artifact_coverage_status),
    ("OBS_TEST_NO_RUNTIME_CHANGE", no_runtime_change_status),
    ("OBS_TEST_NO_CONFIG_CHANGE", no_config_change_status),
    ("OBS_TEST_BODY_NOT_PRINTED", body_not_printed_status),
    ("OBS_TEST_SECRET_SAFE", secret_safe_status),
]:
    detail(f"{label}={status}")
    if status != "PASS":
        fail(f"{label} PASS degil")

detail(f"OBS_TEST_METRIC_EVIDENCE_TOTAL={metric_total}")
detail(f"OBS_TEST_GATE_FAILURE_COUNT={len(gate_failures)}")
detail(f"OBS_TEST_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
detail(f"OBS_TEST_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
detail(f"OBS_TEST_METRIC_MISSING_COUNT={len(metric_missing)}")

matrix_lines = [
    "gate\tstatus\tnote",
    f"baseline\t{baseline_status}\t22.1 observability baseline",
    f"metrics\t{metrics_status}\t22.2 metrics scrape readiness",
    f"logs\t{logs_status}\t22.3 logs/loki readiness",
    f"traces\t{traces_status}\t22.4 traces/tempo readiness",
    f"alerts\t{alerts_status}\t22.5 alert rule catalog",
    f"ops_console\t{ops_status}\t22.6 ops console signal contract",
    ...
    "ops_console_code_changed\tNO\ttest only",
    "ops_console_api_implemented\tNO\ttest only",
    "ops_console_ui_changed\tNO\ttest only",
...
detail(f"OBS_TEST_MATRIX_LINE_COUNT={len(matrix_lines)}")
detail(f"OBS_TEST_INVENTORY_LINE_COUNT={len(inventory_lines)}")

final_status = "PASS" if not failures else "FAIL"
detail(f"OBS_OPS_TESTS={final_status}")
detail(f"FAZ4B_22_7_FINAL_STATUS={final_status}")

report_lines = [
    "# FAZ 4B / 22.7 - Observability / Ops Console Tests Report",
    "",
    f"Generated at: {now()}",
    "",
    "## Summary",
    *details,
    ...
    f"OBS_OPS_TESTS={final_status}",
    f"FAZ4B_22_7_FINAL_STATUS={final_status}",
    "",
    "## Tool Status",
    *tools,
    "",
    "## Matrix",
    "MATRIX_FILE=docs/phase4/22_7_observability_ops_console_tests_matrix.tsv",
    matrix_file.read_text(errors="ignore").rstrip(),
    "",
    "## Inventory",
    "INVENTORY_FILE=docs/phase4/22_7_observability_ops_console_tests_inventory.tsv",
...
print(f"OBS_TEST_BASELINE={baseline_status}")
print(f"OBS_TEST_METRICS={metrics_status}")
print(f"OBS_TEST_LOGS={logs_status}")
print(f"OBS_TEST_TRACES={traces_status}")
print(f"OBS_TEST_ALERTS={alerts_status}")
print(f"OBS_TEST_OPS_CONSOLE={ops_status}")
print(f"OBS_TEST_ARTIFACT_COVERAGE={artifact_coverage_status}")
print(f"OBS_TEST_NO_RUNTIME_CHANGE={no_runtime_change_status}")
print(f"OBS_TEST_NO_CONFIG_CHANGE={no_config_change_status}")
print(f"OBS_TEST_BODY_NOT_PRINTED={body_not_printed_status}")
print(f"OBS_TEST_SECRET_SAFE={secret_safe_status}")
print(f"OBS_TEST_METRIC_EVIDENCE_TOTAL={metric_total}")
print(f"OBS_TEST_GATE_FAILURE_COUNT={len(gate_failures)}")
print(f"OBS_TEST_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
print(f"OBS_TEST_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
print(f"OBS_TEST_METRIC_MISSING_COUNT={len(metric_missing)}")
...
print(f"OBS_OPS_TESTS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
print(f"FAZ4B_22_7_FINAL_STATUS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
