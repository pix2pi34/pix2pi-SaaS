...
echo "==> 1) Backup alınıyor..."
TS="$(date +%Y%m%d_%H%M%S)"
SRC="cmd/mission-control/mission_control_main.go"
BACK="${SRC}.bak_${TS}"
cp -a "$SRC" "$BACK"
echo "OK ✅ backup: $BACK"

echo "==> 2) mission_control_main.go yeniden yazılıyor (Registry discovery)..."
cat <<'GOEOF' > cmd/mission-control/mission_control_main.go
package main
...
echo "OK ✅ wrote: cmd/mission-control/mission_control_main.go"

echo "==> 3) gofmt..."
gofmt -w cmd/mission-control/mission_control_main.go
echo "OK ✅ gofmt"

echo "==> 4) Build binary..."
go build -o /usr/local/bin/pix2pi-mission-control ./cmd/mission-control
echo "OK ✅ build"

echo "==> 5) Restart service..."
systemctl restart pix2pi-mission-control 2>/dev/null || systemctl restart pix2pi-mission-control.service 2>/dev/null || true
echo "OK ✅ restart attempted"

echo "==> 6) Quick checks..."
echo "--- curl http://localhost:${registryPort}/services ---"
curl -s "http://localhost:${registryPort}/services" || true
echo
echo "--- curl http://localhost:${missionPort}/api/services ---"
curl -s "http://localhost:${missionPort}/api/services" || true
echo
echo "--- ss -ltnp | grep ${missionPort} ---"
ss -ltnp | grep "${missionPort}" || true
