...
REPORT_FILE="$REPORT_DIR/14_2_5_pitr_design_wal_archive_report.md"
PLAN_FILE="$REPORT_DIR/14_2_5_pitr_enable_candidate_plan.sh"
...
RISK_FILE="$(mktemp)"
trap 'rm -f "$DETAILS_FILE" "$ISSUES_FILE" "$TOOL_FILE" "$COMPOSE_FILE_LIST" "$CONTAINER_FILE" "$RISK_FILE"' EXIT
...
find_primary_container() {
  ...
  for c in $containers; do
    ...
    recovery="$(docker exec -e PGPASSWORD="$pass_val" "$c" psql -U "$user_val" -d "$db_val" -Atqc "select pg_is_in_recovery();" 2>/tmp/pix2pi_14_2_5_recovery_err.log || echo "error")"
...
detail "READINESS_REPORT=docs/phase4/14_2_1_db_backup_pitr_readiness_report.md"
detail "RESTORE_REPORT=docs/phase4/14_2_4_restore_drill_test_report.md"
detail "PLAN_FILE=docs/phase4/14_2_5_pitr_enable_candidate_plan.sh"
...
if [ "$RESTIC_FOUND" -ne 1 ]; then
  warn "restic bulunamadi; WAL archive backup korunmasi eksik kalir"
fi

if [ ! -f "$READINESS_REPORT" ]; then
  warn "14.2.1 readiness report bulunamadi"
fi

if [ ! -f "$RESTORE_REPORT" ]; then
  fail "14.2.4 restore drill report bulunamadi; PITR design icin restore drill kaniti gerekir"
fi

RESTORE_DRILL_TEST="$(get_report_value "$RESTORE_REPORT" "RESTORE_DRILL_TEST")"
RESTORED_TABLE_COUNT="$(get_report_value "$RESTORE_REPORT" "RESTORED_TABLE_COUNT")"

detail "RESTORE_DRILL_TEST=$RESTORE_DRILL_TEST"
detail "RESTORED_TABLE_COUNT=$RESTORED_TABLE_COUNT"

if [ "$RESTORE_DRILL_TEST" != "PASS" ]; then
  fail "restore drill PASS degil"
fi
...
if [ "$FAIL_COUNT" -eq 0 ]; then
  if PGCONNECT_TIMEOUT=3 psql "$DB_DSN" -v ON_ERROR_STOP=1 -Atqc "select 1;" >/tmp/pix2pi_14_2_5_psql_ok.log 2>/tmp/pix2pi_14_2_5_psql_err.log; then
...
if [ "$FAIL_COUNT" -eq 0 ]; then
  IN_RECOVERY="$(PGCONNECT_TIMEOUT=3 psql "$DB_DSN" -v ON_ERROR_STOP=1 -Atqc "select pg_is_in_recovery();" 2>/tmp/pix2pi_14_2_5_recovery_err.log || echo "error")"
...
if [ -n "$PRIMARY_CONTAINER" ]; then
  docker inspect "$PRIMARY_CONTAINER" --format '{{range .Mounts}}{{println .Source "->" .Destination}}{{end}}' 2>/dev/null > "$CONTAINER_FILE" || true
fi

find "$ROOT_DIR" -maxdepth 4 \( -name 'docker-compose*.yml' -o -name 'docker-compose*.yaml' -o -name 'compose*.yml' -o -name 'compose*.yaml' \) 2>/dev/null | sort > "$COMPOSE_FILE_LIST" || true
...
if [ "$PITR_CURRENT_READY" != "YES" ]; then
  warn "PITR su an aktif degil; 14.2.6 enable gate gerekir"
fi

if [ "$ARCHIVE_MODE_READY" != "YES" ]; then
  risk "RISK_ARCHIVE_MODE_OFF=archive_mode off oldugu icin WAL dosyalari arsivlenmiyor"
fi

if [ "$ARCHIVE_COMMAND_READY" != "YES" ]; then
  risk "RISK_ARCHIVE_COMMAND_DISABLED=archive_command disabled oldugu icin PITR noktasi olusmaz"
fi

if [ "$COMPOSE_FILE_COUNT" -eq 0 ]; then
  risk "RISK_COMPOSE_FILE_NOT_FOUND=Docker compose dosyasi otomatik bulunamadi; enable gate manuel compose path ister"
  warn "docker compose file otomatik bulunamadi"
fi

if [ ! -d "$ROOT_DIR/backups/db" ]; then
  risk "RISK_BACKUP_DB_DIR_MISSING=backups/db dizini yok"
fi

if [ "$RESTIC_FOUND" -ne 1 ]; then
  risk "RISK_RESTIC_NOT_FOUND=restic bulunamadi; WAL arsivi offsite korunamayabilir"
fi
...
detail "PITR_ENABLE_CANDIDATE_PLAN_CREATED=YES"
detail "PITR_ENABLE_CANDIDATE_PLAN_BLOCKED_BY_DEFAULT=YES"

{
  echo "# FAZ 4 / 14.2.5 - PITR Design / WAL Archive Plan Report"
  echo
  ...
  if [ "$FAIL_COUNT" -eq 0 ]; then
    echo "PITR_DESIGN_WAL_ARCHIVE_PLAN=PASS"
  else
    echo "PITR_DESIGN_WAL_ARCHIVE_PLAN=FAIL"
  fi
  ...
  if [ -s "$CONTAINER_FILE" ]; then
    ...
    echo "primary container mount bilgisi yok"
  fi
  ...
  if [ -s "$COMPOSE_FILE_LIST" ]; then
    ...
    echo "compose file candidate yok"
  fi
...
echo "ARCHIVE_MODE_READY=$ARCHIVE_MODE_READY"
echo "ARCHIVE_COMMAND_READY=$ARCHIVE_COMMAND_READY"
