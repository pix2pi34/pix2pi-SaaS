...
CONFIG_FILE="${CONFIG_FILE:-configs/faz4r/faz_4_16_5_4_pilot_rollback_karari_akisi.v1.json}"
ROLLBACK_FILE="${ROLLBACK_FILE:-configs/faz4r/pilot_rollback_decision_flow.controlled_pilot.v1.json}"
INPUT_FILE="${INPUT_FILE:-}"

fail() {
  echo "PILOT_ROLLBACK_DECISION_ERROR=$1"
  exit 1
}
...
if [ ! -f "$ROLLBACK_FILE" ]; then
  fail "ROLLBACK_FILE_NOT_FOUND"
fi

if [ -z "$INPUT_FILE" ]; then
  INPUT_FILE="$ROLLBACK_FILE"
fi
...
rollback_artifact = json.loads(rollback_path.read_text())
...
require(config.get("phase_no") == 216, "CONFIG_PHASE_NO_INVALID")
require(config.get("step") == "FAZ_4_16_5_4", "CONFIG_STEP_INVALID")
...
policy = config.get("rollback_decision_policy", {})
required_rules = set(config.get("required_decision_rules", []))

require(payload.get("rollback_decision_status") == policy.get("rollback_decision_status_required"), "ROLLBACK_DECISION_STATUS_NOT_READY")
require(payload.get("rollback_decision_mode") == policy.get("rollback_decision_mode_required"), "ROLLBACK_DECISION_MODE_NOT_CONTROLLED_PILOT")
...
if isinstance(rules, list):
    for idx, rule in enumerate(rules, start=1):
        ...
        require(non_empty(area), f"{prefix}_DECISION_AREA_REQUIRED")
...
require(summary.get("rollback_decision_result") == policy.get("rollback_decision_result_required"), "ROLLBACK_DECISION_RESULT_INVALID")
require(summary.get("owner_approval_matrix_status") == policy.get("owner_approval_matrix_status_required"), "OWNER_APPROVAL_MATRIX_NOT_READY")
require(summary.get("recovery_validation_plan_status") == policy.get("recovery_validation_plan_status_required"), "RECOVERY_VALIDATION_PLAN_NOT_READY")
require(summary.get("communication_precheck_status") == policy.get("communication_precheck_status_required"), "COMMUNICATION_PRECHECK_NOT_READY")
require(summary.get("decision_log_status") == policy.get("decision_log_status_required"), "DECISION_LOG_STATUS_NOT_READY")
...
require(controls.get("rollback_decision_result") == "NO_ROLLBACK_REQUIRED", "CONTROL_ROLLBACK_DECISION_RESULT_INVALID")
require(controls.get("owner_approval_matrix_status") == "READY", "CONTROL_OWNER_APPROVAL_MATRIX_NOT_READY")
require(controls.get("recovery_validation_plan_status") == "READY", "CONTROL_RECOVERY_VALIDATION_PLAN_NOT_READY")
require(controls.get("communication_precheck_status") == "READY", "CONTROL_COMMUNICATION_PRECHECK_NOT_READY")
require(controls.get("decision_log_status") == "READY", "CONTROL_DECISION_LOG_NOT_READY")
...
if missing_count == 0 and required_fail_count == 0 and critical_issue_count == 0 and open_blocker_count == 0:
    require(summary.get("rollback_decision_flow_result") == "PASS", "ROLLBACK_DECISION_FLOW_RESULT_SHOULD_BE_PASS")
else:
    require(summary.get("rollback_decision_flow_result") == "FAIL", "ROLLBACK_DECISION_FLOW_RESULT_SHOULD_BE_FAIL")
...
if errors:
    print("PILOT_ROLLBACK_DECISION_STATUS=FAIL")
    print(f"PILOT_ROLLBACK_DECISION_ERROR_COUNT={len(errors)}")
    for error in errors:
        print(f"PILOT_ROLLBACK_DECISION_FAIL={error}")
    sys.exit(1)

print("PILOT_ROLLBACK_DECISION_STATUS=PASS")
print(f"PILOT_ROLLBACK_DECISION_TENANT_ID={tenant.get('tenant_id')}")
print(f"PILOT_ROLLBACK_DECISION_TOTAL_RULE_COUNT={total_rule_count}")
print(f"PILOT_ROLLBACK_DECISION_READY_RULE_COUNT={ready_count}")
print(f"PILOT_ROLLBACK_DECISION_MISSING_RULE_COUNT={missing_count}")
print(f"PILOT_ROLLBACK_DECISION_REQUIRED_FAIL_COUNT={summary_required_fail_count}")
print(f"PILOT_ROLLBACK_DECISION_CRITICAL_ISSUE_COUNT={critical_issue_count}")
print(f"PILOT_ROLLBACK_DECISION_OPEN_BLOCKER_COUNT={open_blocker_count}")
print(f"PILOT_ROLLBACK_DECISION_RESULT={summary.get('rollback_decision_result')}")
print("PILOT_ROLLBACK_DECISION_MODE=CONTROLLED_PILOT")
print("ROLLBACK_SIGNAL_STATUS=CLEAR")
print("NO_REAL_ROLLBACK_EXECUTION=true")
print("PILOT_ROLLBACK_DECISION_EXTERNAL_POLICY=CLOSED")
PY_EOF
