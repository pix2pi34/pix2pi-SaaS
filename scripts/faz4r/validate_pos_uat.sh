...
CONFIG_FILE="${CONFIG_FILE:-configs/faz4r/faz_4_16_3_2_pos_uat.v1.json}"
UAT_FILE="${UAT_FILE:-configs/faz4r/pos_uat.controlled_pilot.v1.json}"
INPUT_FILE="${INPUT_FILE:-}"

fail() {
  echo "POS_UAT_ERROR=$1"
  exit 1
}
...
require(config.get("phase_no") == 205, "CONFIG_PHASE_NO_INVALID")
require(config.get("step") == "FAZ_4_16_3_2", "CONFIG_STEP_INVALID")
...
require(payload.get("pos_mode") == policy.get("pos_mode_required"), "POS_MODE_NOT_DRY_RUN")
...
require(summary.get("cashier_session_status") == policy.get("cashier_session_status_required"), "CASHIER_SESSION_STATUS_NOT_PASS")
require(summary.get("sale_dry_run_status") == policy.get("sale_dry_run_status_required"), "SALE_DRY_RUN_STATUS_NOT_PASS")
require(summary.get("payment_provider_status") == policy.get("payment_provider_status_required"), "PAYMENT_PROVIDER_STATUS_NOT_CLOSED")
...
if errors:
    print("POS_UAT_STATUS=FAIL")
    print(f"POS_UAT_ERROR_COUNT={len(errors)}")
    for error in errors:
        print(f"POS_UAT_FAIL={error}")
    sys.exit(1)

print("POS_UAT_STATUS=PASS")
print(f"POS_UAT_TENANT_ID={tenant.get('tenant_id')}")
print(f"POS_UAT_TOTAL_CASE_COUNT={total_case_count}")
print(f"POS_UAT_PASS_COUNT={pass_count}")
print(f"POS_UAT_FAIL_COUNT={fail_count}")
print(f"POS_UAT_REQUIRED_FAIL_COUNT={required_fail_count}")
print(f"POS_UAT_CRITICAL_ISSUE_COUNT={critical_issue_count}")
print(f"POS_UAT_RESULT={summary.get('uat_result')}")
print("POS_UAT_MODE=CONTROLLED_PILOT")
print("POS_MODE=DRY_RUN")
print("POS_UAT_PAYMENT_PROVIDER=CLOSED")
print("POS_UAT_EXTERNAL_POLICY=CLOSED")
PY_EOF
