...
CONFIG_FILE="${CONFIG_FILE:-configs/faz4r/faz_4_16_8_4_go_no_go_karari.v1.json}"
DECISION_FILE="${DECISION_FILE:-configs/faz4r/go_no_go_decision.controlled_pilot.v1.json}"
INPUT_FILE="${INPUT_FILE:-}"

fail() {
  echo "GO_NO_GO_DECISION_ERROR=$1"
  exit 1
}

if [ ! -f "$CONFIG_FILE" ]; then fail "CONFIG_FILE_NOT_FOUND"; fi
if [ ! -f "$DECISION_FILE" ]; then fail "DECISION_FILE_NOT_FOUND"; fi
if [ -z "$INPUT_FILE" ]; then INPUT_FILE="$DECISION_FILE"; fi
...
require(config.get("phase_no") == 234, "CONFIG_PHASE_NO_INVALID")
require(config.get("step") == "FAZ_4_16_8_4", "CONFIG_STEP_INVALID")
...
required_items = set(config.get("required_decision_items", []))

require(payload.get("go_no_go_decision_status") == policy.get("go_no_go_decision_status_required"), "GO_NO_GO_DECISION_STATUS_NOT_READY")
require(payload.get("go_no_go_decision_mode") == policy.get("go_no_go_decision_mode_required"), "GO_NO_GO_DECISION_MODE_NOT_CONTROLLED_PILOT")
require(payload.get("decision_result") == policy.get("decision_result_required"), "DECISION_RESULT_NOT_GO")
...
if isinstance(items, list):
    for idx, item in enumerate(items, start=1):
        ...
        if required is True:
            require(status == policy.get("required_item_status_required"), f"REQUIRED_DECISION_ITEM_NOT_READY:{code}")
...
require(not missing_items, "REQUIRED_DECISION_ITEMS_MISSING:" + ",".join(missing_items))
require(len(provided_items) == len(provided_set), "DUPLICATE_DECISION_ITEM_CODE_FOUND")
...
require(summary.get("critical_issue_reset_status") == policy.get("critical_issue_reset_status_required"), "CRITICAL_ISSUE_RESET_STATUS_NOT_PASS")
...
require(summary.get("decision_result") == policy.get("decision_result_required"), "SUMMARY_DECISION_RESULT_NOT_GO")
...
require(controls.get("critical_issue_reset_status") == "PASS", "CONTROL_CRITICAL_ISSUE_RESET_NOT_PASS")
...
require(controls.get("runtime_health_readiness_status") == "READY", "CONTROL_RUNTIME_HEALTH_NOT_READY")
...
require(controls.get("decision_result_status") == "GO", "CONTROL_DECISION_RESULT_NOT_GO")
...
if missing_count == 0 and required_fail_count == 0 and summary.get("critical_issue_count") == 0 and summary.get("open_blocker_count") == 0 and summary.get("decision_result") == "GO":
    require(summary.get("go_no_go_decision_result") == "PASS", "GO_NO_GO_DECISION_RESULT_SHOULD_BE_PASS")
else:
    require(summary.get("go_no_go_decision_result") == "FAIL", "GO_NO_GO_DECISION_RESULT_SHOULD_BE_FAIL")
...
if errors:
    print("GO_NO_GO_DECISION_STATUS=FAIL")
    print(f"GO_NO_GO_DECISION_ERROR_COUNT={len(errors)}")
    for error in errors:
        print(f"GO_NO_GO_DECISION_FAIL={error}")
    sys.exit(1)

print("GO_NO_GO_DECISION_STATUS=PASS")
print(f"GO_NO_GO_DECISION_TENANT_ID={tenant.get('tenant_id')}")
print(f"GO_NO_GO_DECISION_TOTAL_ITEM_COUNT={total_item_count}")
print(f"GO_NO_GO_DECISION_READY_ITEM_COUNT={ready_count}")
print(f"GO_NO_GO_DECISION_MISSING_ITEM_COUNT={missing_count}")
print(f"GO_NO_GO_DECISION_REQUIRED_FAIL_COUNT={summary_required_fail_count}")
...
print("PILOT_KPI_STATUS=PASS")
print("REHEARSAL_REPORT_STATUS=PASS")
print("OWNER_APPROVAL_STATUS=APPROVED")
print("DECISION_RESULT=GO")
print(f"GO_NO_GO_DECISION_RESULT={summary.get('go_no_go_decision_result')}")
print("GO_NO_GO_DECISION_MODE=CONTROLLED_PILOT")
print("NO_PRODUCTION_LAUNCH=true")
print("NO_LIVE_EXTERNAL_PROVIDER_ACTIVATION=true")
print("GO_NO_GO_DECISION_EXTERNAL_POLICY=CLOSED")
PY_EOF
