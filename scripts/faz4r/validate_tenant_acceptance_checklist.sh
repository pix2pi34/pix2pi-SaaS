...
CONFIG_FILE="${CONFIG_FILE:-configs/faz4r/faz_4_16_1_5_tenant_acceptance_checklist.v1.json}"
INPUT_FILE="${INPUT_FILE:-}"

fail() {
  echo "TENANT_ACCEPTANCE_ERROR=$1"
  exit 1
}
...
require(config.get("phase_no") == 193, "CONFIG_PHASE_NO_INVALID")
require(config.get("step") == "FAZ_4_16_1_5", "CONFIG_STEP_INVALID")
...
policy = config.get("acceptance_policy", {})
required_items = set(config.get("required_checklist_items", []))

tenant = payload.get("tenant", {})
summary = payload.get("summary", {})
checklist = payload.get("checklist", [])

require(isinstance(tenant.get("tenant_id"), str) and tenant.get("tenant_id"), "TENANT_ID_REQUIRED")
require(isinstance(tenant.get("tenant_name"), str) and tenant.get("tenant_name"), "TENANT_NAME_REQUIRED")
...
require(summary.get("readmodel_reporting_status") == policy.get("readmodel_reporting_status_required"), "READMODEL_REPORTING_STATUS_NOT_PASS")
...
for item in checklist:
    ...
    if required is True:
        require(status == "PASS", f"REQUIRED_ITEM_NOT_PASS:{code}")
        require(isinstance(evidence_ref, str) and len(evidence_ref.strip()) > 0, f"REQUIRED_EVIDENCE_MISSING:{code}")

missing_items = sorted(required_items - provided_codes)
require(not missing_items, "REQUIRED_CHECKLIST_ITEMS_MISSING:" + ",".join(missing_items))

for required_code in required_items:
    matching = [item for item in checklist if item.get("code") == required_code]
    require(len(matching) == 1, f"REQUIRED_ITEM_DUPLICATE_OR_MISSING:{required_code}")
    if matching:
        item = matching[0]
        require(item.get("required") is True, f"REQUIRED_FLAG_FALSE:{required_code}")
        require(item.get("status") == "PASS", f"REQUIRED_ITEM_NOT_PASS:{required_code}")
...
if errors:
    print("TENANT_ACCEPTANCE_CHECKLIST_STATUS=FAIL")
    for error in errors:
        print(f"ACCEPTANCE_FAIL={error}")
    sys.exit(1)

print("TENANT_ACCEPTANCE_CHECKLIST_STATUS=PASS")
print(f"TENANT_ACCEPTANCE_TENANT_ID={tenant.get('tenant_id')}")
print("TENANT_ACCEPTANCE_REQUIRED_ITEMS_STATUS=PASS")
print("TENANT_ACCEPTANCE_EVIDENCE_STATUS=PASS")
print("TENANT_ACCEPTANCE_EXTERNAL_POLICY=CLOSED")
PY_EOF
