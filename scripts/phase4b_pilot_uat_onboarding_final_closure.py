...
standard_file = report_dir / "16_7_pilot_uat_onboarding_final_closure_standard.md"
report_file = report_dir / "16_7_pilot_uat_onboarding_final_closure_report.md"
matrix_file = report_dir / "16_7_pilot_uat_onboarding_final_closure_matrix.tsv"
inventory_file = report_dir / "16_7_pilot_uat_onboarding_final_closure_inventory.tsv"
closure_file = report_dir / "16_pilot_uat_onboarding_final_closure_report.md"
...
reports = {
    ...
    "16.6": report_dir / "16_6_pilot_uat_onboarding_tests_report.md",
...
domain_keys = {
    ...
    "16.6": "PILOT_UAT_ONBOARDING_TESTS",
}
...
required_pass_keys = {
    ...
    "16.6": [
        "PILOT_TEST_BASELINE",
        "PILOT_TEST_TENANT_READINESS",
        "PILOT_TEST_UAT_EXECUTION",
        "PILOT_TEST_DATA_READINESS",
        "PILOT_TEST_GO_NO_GO",
        "PILOT_TEST_ARTIFACT_COVERAGE",
        "PILOT_TEST_NO_RUNTIME_CHANGE",
        "PILOT_TEST_NO_CONFIG_CHANGE",
        "PILOT_TEST_SECRET_SAFE",
    ],
}
...
required_no_keys = {
    "16.1": base_no_keys,
    "16.2": base_no_keys + ["TENANT_CREATED", "USER_CREATED", "PASSWORD_CREATED", "TOKEN_CREATED"],
    "16.3": base_no_keys + [
        "TENANT_CREATED", "USER_CREATED", "PASSWORD_CREATED", "TOKEN_CREATED",
        "UAT_EXECUTED", "REAL_SALE_CREATED", "REAL_STOCK_MUTATED",
        "REAL_ACCOUNTING_ENTRY_CREATED",
    ],
    "16.4": base_no_keys + [
        "SAMPLE_DATA_INSERTED", "REAL_CUSTOMER_DATA_CREATED", "REAL_PRODUCT_CREATED",
        "REAL_STOCK_MUTATED", "REAL_SALE_CREATED", "REAL_ACCOUNTING_ENTRY_CREATED",
        "DATA_IMPORT_EXECUTED", "FILE_EXPORT_EXECUTED",
    ],
    "16.5": base_no_keys + [
        "ROLLOUT_EXECUTED", "GO_LIVE_SWITCHED", "PRODUCTION_TRAFFIC_CHANGED",
        "TENANT_ENABLED_FOR_LIVE", "REAL_CUSTOMER_NOTIFIED", "UAT_EXECUTED",
        "SAMPLE_DATA_INSERTED", "REAL_CUSTOMER_DATA_CREATED", "REAL_PRODUCT_CREATED",
        "REAL_STOCK_MUTATED", "REAL_SALE_CREATED", "REAL_ACCOUNTING_ENTRY_CREATED",
        "DATA_IMPORT_EXECUTED", "FILE_EXPORT_EXECUTED",
    ],
    "16.6": base_no_keys + [
        "ROLLOUT_EXECUTED", "GO_LIVE_SWITCHED", "PRODUCTION_TRAFFIC_CHANGED",
        "TENANT_ENABLED_FOR_LIVE", "REAL_CUSTOMER_NOTIFIED", "TENANT_CREATED",
        "USER_CREATED", "PASSWORD_CREATED", "TOKEN_CREATED", "UAT_EXECUTED",
        "SAMPLE_DATA_INSERTED", "REAL_CUSTOMER_DATA_CREATED", "REAL_PRODUCT_CREATED",
        "REAL_STOCK_MUTATED", "REAL_SALE_CREATED", "REAL_ACCOUNTING_ENTRY_CREATED",
        "DATA_IMPORT_EXECUTED", "FILE_EXPORT_EXECUTED",
    ],
...
metric_keys = {
    ...
    "16.6": [
        "PILOT_TEST_METRIC_EVIDENCE_TOTAL",
        "PILOT_TEST_GATE_FAILURE_COUNT",
        "PILOT_TEST_NO_CHANGE_FAILURE_COUNT",
        "PILOT_TEST_ARTIFACT_MISSING_COUNT",
        "PILOT_TEST_METRIC_MISSING_COUNT",
    ],
}
...
artifact_sets = {
    ...
    "16.6": [
        "docs/phase4/16_6_pilot_uat_onboarding_tests_standard.md",
        "docs/phase4/16_6_pilot_uat_onboarding_tests_report.md",
        "docs/phase4/16_6_pilot_uat_onboarding_tests_matrix.tsv",
        "docs/phase4/16_6_pilot_uat_onboarding_tests_inventory.tsv",
...
detail("VALIDATION_MODE=PILOT_UAT_ONBOARDING_FINAL_CLOSURE_ONLY")

for tool in ["python3", "grep", "wc"]:
    tool_status(tool)

if not standard_file.exists():
    fail("16.7 standard doc yok")
...
for block in ["16.1", "16.2", "16.3", "16.4", "16.5", "16.6"]:
    report = reports[block]
    final_status = get_value(report, final_keys[block])
    domain_status = get_value(report, domain_keys[block])

    detail(f"PILOT_FINAL_{block.replace('.', '_')}_FINAL_STATUS={final_status}")
    detail(f"PILOT_FINAL_{block.replace('.', '_')}_{domain_keys[block]}={domain_status}")
    ...
    for key in required_pass_keys[block]:
        ...
        detail(f"PILOT_FINAL_{block.replace('.', '_')}_{key}={value}")
        if value != "PASS":
    ...
    detail(f"PILOT_FINAL_{block.replace('.', '_')}_ARTIFACT_EXPECTED_COUNT={expected_artifact_count}")
    detail(f"PILOT_FINAL_{block.replace('.', '_')}_ARTIFACT_EXISTING_COUNT={existing_artifact_count}")
...
if gate_failures:
    fail("pilot final gate failure: " + ",".join(gate_failures[:100]))

if no_change_failures:
    fail("pilot final no-change failure: " + ",".join(no_change_failures[:100]))

if artifact_missing:
    fail("pilot final artifact eksik: " + ",".join(artifact_missing[:100]))

if metric_missing:
    fail("pilot final metric evidence eksik: " + ",".join(metric_missing[:100]))
...
tests_status = block_results["16.6"]["status"]
...
for label, status in [
    ("PILOT_FINAL_BASELINE", baseline_status),
    ("PILOT_FINAL_TENANT_READINESS", tenant_readiness_status),
    ("PILOT_FINAL_UAT_EXECUTION", uat_execution_status),
    ("PILOT_FINAL_DATA_READINESS", data_readiness_status),
    ("PILOT_FINAL_GO_NO_GO", go_no_go_status),
    ("PILOT_FINAL_TESTS", tests_status),
    ("PILOT_FINAL_ARTIFACT_COVERAGE", artifact_coverage_status),
    ("PILOT_FINAL_NO_RUNTIME_CHANGE", no_runtime_change_status),
    ("PILOT_FINAL_NO_CONFIG_CHANGE", no_config_change_status),
    ("PILOT_FINAL_SECRET_SAFE", secret_safe_status),
]:
    detail(f"{label}={status}")
    if status != "PASS":
        fail(f"{label} PASS degil")

detail(f"PILOT_FINAL_METRIC_EVIDENCE_TOTAL={metric_total}")
detail(f"PILOT_FINAL_GATE_FAILURE_COUNT={len(gate_failures)}")
detail(f"PILOT_FINAL_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
detail(f"PILOT_FINAL_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
detail(f"PILOT_FINAL_METRIC_MISSING_COUNT={len(metric_missing)}")

matrix_lines = [
    "gate\tstatus\tnote",
    f"baseline\t{baseline_status}\t16.1 pilot baseline final",
    f"tenant_readiness\t{tenant_readiness_status}\t16.2 tenant readiness final",
    f"uat_execution\t{uat_execution_status}\t16.3 UAT execution final",
    f"data_readiness\t{data_readiness_status}\t16.4 pilot data readiness final",
    f"go_no_go\t{go_no_go_status}\t16.5 go/no-go final",
    f"pilot_tests\t{tests_status}\t16.6 pilot UAT tests final",
...
detail(f"PILOT_FINAL_MATRIX_LINE_COUNT={len(matrix_lines)}")
detail(f"PILOT_FINAL_INVENTORY_LINE_COUNT={len(inventory_lines)}")

final_status = "PASS" if not failures else "FAIL"
detail(f"PILOT_UAT_ONBOARDING_FINAL_CLOSURE={final_status}")
detail(f"FAZ4B_16_7_FINAL_STATUS={final_status}")
detail(f"FAZ4B_16_FINAL_STATUS={final_status}")
...
report_lines = [
    ...
    *details,
    ...
    f"PILOT_UAT_ONBOARDING_FINAL_CLOSURE={final_status}",
    f"FAZ4B_16_7_FINAL_STATUS={final_status}",
    f"FAZ4B_16_FINAL_STATUS={final_status}",
    "",
    "## Tool Status",
    *tools,
    "",
...
closure_lines = [
    ...
    "",
    "## Closed Items",
    f"16.1 Pilot / UAT / Onboarding baseline / scope inventory={baseline_status}",
    f"16.2 Pilot tenant readiness / role & onboarding contract={tenant_readiness_status}",
    f"16.3 UAT scenario execution contract={uat_execution_status}",
    f"16.4 Pilot data readiness / sample dataset contract={data_readiness_status}",
    f"16.5 Go / No-Go rollout gate={go_no_go_status}",
    f"16.6 Pilot / UAT / Onboarding tests={tests_status}",
    f"16.7 Pilot / UAT / Onboarding final closure={final_status}",
    "",
    "## Final Gates",
    f"PILOT_FINAL_BASELINE={baseline_status}",
    f"PILOT_FINAL_TENANT_READINESS={tenant_readiness_status}",
    f"PILOT_FINAL_UAT_EXECUTION={uat_execution_status}",
    f"PILOT_FINAL_DATA_READINESS={data_readiness_status}",
    f"PILOT_FINAL_GO_NO_GO={go_no_go_status}",
    f"PILOT_FINAL_TESTS={tests_status}",
    f"PILOT_FINAL_ARTIFACT_COVERAGE={artifact_coverage_status}",
    f"PILOT_FINAL_NO_RUNTIME_CHANGE={no_runtime_change_status}",
    f"PILOT_FINAL_NO_CONFIG_CHANGE={no_config_change_status}",
    f"PILOT_FINAL_SECRET_SAFE={secret_safe_status}",
    f"PILOT_FINAL_METRIC_EVIDENCE_TOTAL={metric_total}",
    "",
...
print(f"PILOT_FINAL_BASELINE={baseline_status}")
print(f"PILOT_FINAL_TENANT_READINESS={tenant_readiness_status}")
print(f"PILOT_FINAL_UAT_EXECUTION={uat_execution_status}")
print(f"PILOT_FINAL_DATA_READINESS={data_readiness_status}")
print(f"PILOT_FINAL_GO_NO_GO={go_no_go_status}")
print(f"PILOT_FINAL_TESTS={tests_status}")
print(f"PILOT_FINAL_ARTIFACT_COVERAGE={artifact_coverage_status}")
print(f"PILOT_FINAL_NO_RUNTIME_CHANGE={no_runtime_change_status}")
print(f"PILOT_FINAL_NO_CONFIG_CHANGE={no_config_change_status}")
print(f"PILOT_FINAL_SECRET_SAFE={secret_safe_status}")
print(f"PILOT_FINAL_METRIC_EVIDENCE_TOTAL={metric_total}")
print(f"PILOT_FINAL_GATE_FAILURE_COUNT={len(gate_failures)}")
print(f"PILOT_FINAL_NO_CHANGE_FAILURE_COUNT={len(no_change_failures)}")
print(f"PILOT_FINAL_ARTIFACT_MISSING_COUNT={len(artifact_missing)}")
print(f"PILOT_FINAL_METRIC_MISSING_COUNT={len(metric_missing)}")
...
print(f"PILOT_UAT_ONBOARDING_FINAL_CLOSURE={final_status} {'✅' if final_status == 'PASS' else '❌'}")
print(f"FAZ4B_16_7_FINAL_STATUS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
print(f"FAZ4B_16_FINAL_STATUS={final_status} {'✅' if final_status == 'PASS' else '❌'}")
