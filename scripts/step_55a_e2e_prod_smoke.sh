...
QUERY_COUNT_URL="http://127.0.0.1:9010/api/query/users"
QUERY_LIST_URL="http://127.0.0.1:9010/api/query/users/list?limit=10"
...
echo
echo "===== 2) PRE COUNT ====="
pre_json="$(curl -fsS "$QUERY_COUNT_URL")"
echo "$pre_json"
...
echo
echo "===== 4) WAIT FOR EVENT / POST COUNT ====="
expected_count=$((pre_count + 1))
post_json=""
post_count=""

for i in $(seq 1 20); do
  post_json="$(curl -fsS "$QUERY_COUNT_URL" || true)"
...
if [ "$post_count" != "$expected_count" ]; then
  echo "HATA ❌ beklenen count=$expected_count ama gelen=$post_count"
  exit 1
fi
...
echo "OK ✅ filter bulundu"

echo
echo "===== 8) SON 30 CONSUMER LOG ====="
journalctl -u "$CONSUMER_SERVICE" -n 30 --no-pager

echo
echo "===== 9) SON 30 GATEWAY LOG ====="
journalctl -u "$API_SERVICE" -n 30 --no-pager

echo
echo "OK ✅ step_55a prod e2e smoke gecti"
