...
ALTER TABLE org.legal_entities ADD COLUMN IF NOT EXISTS status text DEFAULT 'ACTIVE';
...
UPDATE org.legal_entities SET status='ACTIVE' WHERE status IS NULL OR btrim(status)='';
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_org_legal_entities_id_tenant_id
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE org.legal_entities
      ...
      CHECK (status IN ('ACTIVE','PASSIVE','SUSPENDED','DELETED')) NOT VALID;
  END IF;
...
ALTER TABLE org.legal_entity_addresses ADD COLUMN IF NOT EXISTS status text DEFAULT 'ACTIVE';
...
UPDATE org.legal_entity_addresses SET status='ACTIVE' WHERE status IS NULL OR btrim(status)='';
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE org.legal_entity_addresses
      ...
      CHECK (status IN ('ACTIVE','PASSIVE','DELETED')) NOT VALID;
  END IF;
...
BEGIN
  IF EXISTS (
    ...
    SELECT
      ...
      jsonb_build_object('phase','FAZ_1_1_2','source','legal_entity_model'),
      now()
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema='app_dictionary'
      AND table_name='field_contracts'
  ) THEN
    ...
    SELECT
      ...
      CASE
        ...
        ELSE upper(coalesce(c.data_type,'UNKNOWN'))
      END,
      (c.is_nullable='YES'),
      CASE
        WHEN c.column_name IN ('id','tenant_id','business_code','legal_name','tax_number','tax_office','address_line','created_at','updated_at') THEN 'REQUIRED_STANDARD'
        WHEN c.column_name IN ('legal_entity_id','branch_id','created_by','updated_by','deleted_at','mersis_no','phone','email') THEN 'OPTIONAL_CONTEXTUAL'
        WHEN c.column_default IS NOT NULL THEN 'SYSTEM_DEFAULT'
        WHEN c.is_nullable='NO' THEN 'REQUIRED_DB'
        ELSE 'OPTIONAL'
      END,
      c.column_default,
    ...
    FROM information_schema.columns c
    WHERE c.table_schema='org'
      AND c.table_name IN ('legal_entities','legal_entity_addresses')
