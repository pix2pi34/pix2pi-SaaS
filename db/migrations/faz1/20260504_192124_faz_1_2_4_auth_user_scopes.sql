-- FAZ 1-2.4 auth.user_scopes canonical runtime model
-- Tenant/legal entity/branch/accountant scope + expiration + audit + RLS tests
...
SET
  scope_id = coalesce(nullif(scope_id, ''), auth.security_generated_id('uscope')),
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_user_scopes_scope_value_required') THEN
    ALTER TABLE auth.user_scopes
      ...
      CHECK (scope_value IS NOT NULL AND length(btrim(scope_value)) > 0) NOT VALID;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_user_scopes_user_required') THEN
    ALTER TABLE auth.user_scopes
      ...
      CHECK (user_id IS NOT NULL AND length(btrim(user_id)) > 0) NOT VALID;
  END IF;
...
BEGIN
  ...
  VALUES (
    ...
    p_scope_type,
    v_scope_value,
    nullif(btrim(coalesce(p_legal_entity_id, '')), ''),
    nullif(btrim(coalesce(p_branch_id, '')), ''),
    nullif(btrim(coalesce(p_accountant_company_id, '')), ''),
    'ACTIVE',
...
BEGIN
  ...
  VALUES (
    ...
    'ACTIVE',
    'REVOKED',
    jsonb_build_object('scope_type', v_scope.scope_type, 'scope_value', v_scope.scope_value)
  );
END;
$$;
