-- FAZ 1-2.4 auth.user_scopes canonical runtime model FIX V2
-- Fix: existing uuid columns are normalized with ::text before text constraints/functions.
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'ALTER TABLE auth.user_scopes ALTER COLUMN %I TYPE text USING %I::text',
      r.column_name,
      r.column_name
    );
  END LOOP;
