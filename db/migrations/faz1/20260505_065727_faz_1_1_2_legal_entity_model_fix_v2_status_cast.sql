...
UPDATE org.legal_entities SET status='ACTIVE' WHERE status IS NULL OR btrim(status::text)='';
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE org.legal_entities
      ...
      CHECK (status::text IN ('ACTIVE','PASSIVE','SUSPENDED','DELETED')) NOT VALID;
  END IF;
...
UPDATE org.legal_entity_addresses SET status='ACTIVE' WHERE status IS NULL OR btrim(status::text)='';
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE org.legal_entity_addresses
      ...
      CHECK (status::text IN ('ACTIVE','PASSIVE','DELETED')) NOT VALID;
  END IF;
...
BEGIN
  IF EXISTS (
    ...
    SELECT
      ...
      jsonb_build_object('phase','FAZ_1_1_2_FIX_V2','source','legal_entity_model'),
      now()
