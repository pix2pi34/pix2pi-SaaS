...
CREATE TABLE IF NOT EXISTS org.branches (
  ...
  branch_type text NOT NULL DEFAULT 'PHYSICAL',
  phone text,
  email text,
...
ALTER TABLE org.branches ADD COLUMN IF NOT EXISTS branch_type text DEFAULT 'PHYSICAL';
...
ALTER TABLE org.branches ADD COLUMN IF NOT EXISTS status text DEFAULT 'ACTIVE';
...
UPDATE org.branches SET branch_type='PHYSICAL' WHERE branch_type IS NULL OR btrim(branch_type)='';
...
CREATE INDEX IF NOT EXISTS idx_org_branches_tenant_id
  ON org.branches(tenant_id);

CREATE INDEX IF NOT EXISTS idx_org_branches_legal_entity_id
  ON org.branches(legal_entity_id);

CREATE INDEX IF NOT EXISTS idx_org_branches_scope_key
  ON org.branches(scope_key);

CREATE INDEX IF NOT EXISTS idx_org_branches_status
  ON org.branches(status);
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='ck_org_branches_required_fields'
      AND conrelid='org.branches'::regclass
  ) THEN
    ALTER TABLE org.branches
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='ck_org_branches_branch_type'
      AND conrelid='org.branches'::regclass
  ) THEN
    ALTER TABLE org.branches
      ADD CONSTRAINT ck_org_branches_branch_type
      CHECK (branch_type IN ('PHYSICAL','ONLINE','WAREHOUSE','HEAD_OFFICE','OTHER')) NOT VALID;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='ck_org_branches_status'
      AND conrelid='org.branches'::regclass
  ) THEN
    ALTER TABLE org.branches
...
ALTER TABLE org.branch_addresses ADD COLUMN IF NOT EXISTS address_type text DEFAULT 'PRIMARY';
...
ALTER TABLE org.branch_addresses ADD COLUMN IF NOT EXISTS status text DEFAULT 'ACTIVE';
...
UPDATE org.branch_addresses SET address_type='PRIMARY' WHERE address_type IS NULL OR btrim(address_type)='';
...
CREATE INDEX IF NOT EXISTS idx_org_branch_addresses_tenant_id
  ON org.branch_addresses(tenant_id);

CREATE INDEX IF NOT EXISTS idx_org_branch_addresses_legal_entity_id
  ON org.branch_addresses(legal_entity_id);

CREATE INDEX IF NOT EXISTS idx_org_branch_addresses_branch_id
  ON org.branch_addresses(branch_id);

CREATE INDEX IF NOT EXISTS idx_org_branch_addresses_status
  ON org.branch_addresses(status);
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='ck_org_branch_addresses_required_fields'
      AND conrelid='org.branch_addresses'::regclass
  ) THEN
    ALTER TABLE org.branch_addresses
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='ck_org_branch_addresses_address_type'
      AND conrelid='org.branch_addresses'::regclass
  ) THEN
    ALTER TABLE org.branch_addresses
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname='ck_org_branch_addresses_status'
      AND conrelid='org.branch_addresses'::regclass
  ) THEN
    ALTER TABLE org.branch_addresses
...
BEGIN
  IF EXISTS (
    ...
    SELECT
      ...
      jsonb_build_object('phase','FAZ_1_1_3','source','branch_model'),
      now()
