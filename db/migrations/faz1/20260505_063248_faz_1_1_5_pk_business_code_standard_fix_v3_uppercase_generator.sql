...
AS $$
  SELECT left(app_standard.normalize_business_code(coalesce(prefix, 'CODE')), 16)
         || '_'
         || upper(substr(replace(coalesce(source_id, gen_random_uuid())::text, '-', ''), 1, 12));
$$;
...
BEGIN
  ...
  LOOP
    ...
    IF invalid_count > 0 THEN
      RAISE EXCEPTION 'invalid business_code rows remain in %.% count=%', r.table_schema, r.table_name, invalid_count;
    END IF;
    ...
    IF NOT EXISTS (
      ...
      EXCEPTION WHEN duplicate_object THEN
        NULL;
      WHEN others THEN
        RAISE EXCEPTION 'business_code format check add failed for %.%: %', r.table_schema, r.table_name, SQLERRM;
      END;
    END IF;
  END LOOP;
END $$;
